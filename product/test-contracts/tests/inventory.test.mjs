...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary['reusable-host'], { classes: 31, tests: 194, knownCases: 219, unknownCaseTests: 0 })
  assert.deepEqual(hostArtifact.summary['fresh-host'], { classes: 41, tests: 205, knownCases: 230, unknownCaseTests: 0 })
...
test('host-classification case totals join exactly to the intent inventory', () => {
  ...
  assert.equal(reusable.tests - reusable.classes, 163)
  assert.equal(reusable.knownCases - reusable.classes, 188)
})
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /reusable-host\s+31\s+194\s+219\s+0\s+38\.8%/)
    assert.match(output, /fresh-host\s+41\s+205\s+230\s+0\s+41\.0%/)
    assert.match(output, /Remaining reuse headroom:\s+31 classes, 194 methods, 219 known cases/)
  } finally {
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 500)
  assert.equal(intentArtifact.totals.cases, 550)
  ...
  assert.equal(hostArtifact.totals.knownCases, 550)
