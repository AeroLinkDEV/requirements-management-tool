...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary['fresh-host'], { classes: 44, tests: 291, knownCases: 323, unknownCaseTests: 0 })
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  assert.match(output, /fresh-host\s+44\s+291\s+323\s+0\s+39.8%/)
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 732)
  assert.equal(intentArtifact.totals.cases, 816)
  ...
  assert.equal(hostArtifact.totals.knownCases, 816)
