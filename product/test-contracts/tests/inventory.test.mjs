...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary['fresh-host'], { classes: 41, tests: 204, knownCases: 229, unknownCaseTests: 0 })
  assert.deepEqual(hostArtifact.summary.converted, { classes: 14, tests: 68, knownCases: 68, unknownCaseTests: 0 })
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /reusable-host\s+30\s+186\s+211\s+0\s+40\.5%/)
    assert.match(output, /fresh-host\s+41\s+204\s+229\s+0\s+44\.4%/)
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 459)
  assert.equal(intentArtifact.totals.cases, 509)
  ...
  assert.equal(hostArtifact.totals.knownCases, 509)
