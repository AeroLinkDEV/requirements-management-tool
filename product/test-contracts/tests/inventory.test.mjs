...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /reusable-host\s+45\s+293\s+324\s+0\s+34.7%/)
    assert.match(output, /fresh-host\s+47\s+298\s+330\s+0\s+35.3%/)
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 844)
  assert.equal(intentArtifact.totals.cases, 937)
  ...
  assert.equal(hostArtifact.totals.knownCases, 937)
