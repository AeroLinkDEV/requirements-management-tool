...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /reusable-host\s+57\s+350\s+396\s+0\s+35.6%/)
    assert.match(output, /fresh-host\s+60\s+366\s+430\s+0\s+37.2%/)
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 984)
  assert.equal(intentArtifact.totals.cases, 1124)
  ...
  assert.equal(hostArtifact.totals.knownCases, 1124)
