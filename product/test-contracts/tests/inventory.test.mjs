...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary['fresh-host'], { classes: 44, tests: 278, knownCases: 309, unknownCaseTests: 0 })
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /reusable-host\s+44\s+279\s+310\s+0\s+38.8%/)
    assert.match(output, /fresh-host\s+44\s+278\s+309\s+0\s+38.7%/)
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 719)
  assert.equal(intentArtifact.totals.cases, 802)
  ...
  assert.equal(hostArtifact.totals.knownCases, 802)
