...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary['reusable-host'], { classes: 53, tests: 325, knownCases: 366, unknownCaseTests: 0 })
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /reusable-host\s+53\s+325\s+366\s+0\s+35.7%/)
    assert.match(output, /fresh-host\s+54\s+319\s+359\s+0\s+35.1%/)
    assert.match(output, /Remaining reuse headroom:\s+53 classes, 325 methods, 366 known cases/)
  } finally {
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 910)
  assert.equal(intentArtifact.totals.cases, 1021)
  ...
  assert.equal(hostArtifact.totals.knownCases, 1021)
