...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /fresh-host\s+62\s+380\s+444\s+0\s+38.1%/)
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 998)
  assert.equal(intentArtifact.totals.cases, 1138)
  ...
  assert.equal(hostArtifact.totals.knownCases, 1138)
