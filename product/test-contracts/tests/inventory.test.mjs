...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /reusable-host\s+56\s+347\s+393\s+0\s+37.2%/)
    assert.match(output, /fresh-host\s+54\s+320\s+360\s+0\s+34.3%/)
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 934)
  assert.equal(intentArtifact.totals.cases, 1050)
  ...
  assert.equal(hostArtifact.totals.knownCases, 1050)
