...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary['reusable-host'], { classes: 57, tests: 348, knownCases: 394, unknownCaseTests: 0 })
  // Includes GitLab metadata and Code relationship tests with isolated provider configuration and access-revocation races.
  assert.deepEqual(hostArtifact.summary['fresh-host'], { classes: 60, tests: 365, knownCases: 429, unknownCaseTests: 0 })
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  assert.match(output, /reusable-host\s+57\s+348\s+394\s+0\s+35.5%/)
  assert.match(output, /fresh-host\s+60\s+365\s+429\s+0\s+37.2%/)
    assert.match(output, /Remaining reuse headroom:\s+57 classes, 348 methods, 394 known cases/)
  } finally {
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 980)
  assert.equal(intentArtifact.totals.cases, 1120)
  ...
  assert.equal(hostArtifact.totals.knownCases, 1120)
