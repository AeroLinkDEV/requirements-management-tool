...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary['reusable-host'], { classes: 45, tests: 293, knownCases: 324, unknownCaseTests: 0 })
...
test('host-classification case totals join exactly to the intent inventory', () => {
  ...
  assert.equal(reusable.tests - reusable.classes, 248)
  assert.equal(reusable.knownCases - reusable.classes, 279)
})
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /reusable-host\s+45\s+293\s+324\s+0\s+34.8%/)
    assert.match(output, /fresh-host\s+46\s+295\s+327\s+0\s+35.1%/)
    assert.match(output, /Remaining reuse headroom:\s+45 classes, 293 methods, 324 known cases/)
  } finally {
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 841)
  assert.equal(intentArtifact.totals.cases, 934)
  ...
  assert.equal(hostArtifact.totals.knownCases, 934)
