...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /reusable-host\s+31\s+192\s+217\s+0\s+37\.5%/)
    assert.match(output, /fresh-host\s+42\s+219\s+248\s+0\s+42\.8%/)
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 512)
  assert.equal(intentArtifact.totals.cases, 566)
  ...
  assert.equal(hostArtifact.totals.knownCases, 566)
