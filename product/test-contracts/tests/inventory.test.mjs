...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary['reusable-host'], { classes: 50, tests: 314, knownCases: 349, unknownCaseTests: 0 })
  ...
  assert.deepEqual(hostArtifact.summary.converted, { classes: 30, tests: 251, knownCases: 276, unknownCaseTests: 0 })
...
test('host-classification case totals join exactly to the intent inventory', () => {
  ...
  assert.equal(reusable.tests - reusable.classes, 264)
  assert.equal(reusable.knownCases - reusable.classes, 299)
})
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /reusable-host\s+50\s+314\s+349\s+0\s+36.0%/)
    assert.match(output, /fresh-host\s+48\s+300\s+332\s+0\s+34.4%/)
    assert.match(output, /Remaining reuse headroom:\s+50 classes, 314 methods, 349 known cases/)
  } finally {
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 873)
  assert.equal(intentArtifact.totals.cases, 970)
  ...
  assert.equal(hostArtifact.totals.knownCases, 970)
