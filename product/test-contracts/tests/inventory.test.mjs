...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary['reusable-host'], { classes: 38, tests: 238, knownCases: 267, unknownCaseTests: 0 })
...
test('host-classification case totals join exactly to the intent inventory', () => {
  ...
  assert.equal(reusable.tests - reusable.classes, 200)
  assert.equal(reusable.knownCases - reusable.classes, 229)
})
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /classification\s+classes\s+methods\s+known cases\s+unknown-case methods\s+share of methods/)
     assert.match(output, /reusable-host\s+38\s+238\s+267\s+0\s+40\.2%/)
     assert.match(output, /fresh-host\s+42\s+234\s+263\s+0\s+39\.5%/)
     assert.match(output, /Remaining reuse headroom:\s+38 classes, 238 methods, 267 known cases/)
  } finally {
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 592)
  assert.equal(intentArtifact.totals.cases, 650)
  ...
  assert.equal(hostArtifact.totals.knownCases, 650)
