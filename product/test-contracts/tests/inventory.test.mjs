...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary['fresh-host'], { classes: 46, tests: 296, knownCases: 328, unknownCaseTests: 0 })
...
test('host-classification case totals join exactly to the intent inventory', () => {
  ...
  assert.equal(reusable.tests - reusable.classes, 250)
  assert.equal(reusable.knownCases - reusable.classes, 281)
})
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /reusable-host\s+45\s+295\s+326\s+0\s+35.0%/)
    assert.match(output, /fresh-host\s+46\s+296\s+328\s+0\s+35.1%/)
    assert.match(output, /Remaining reuse headroom:\s+45 classes, 295 methods, 326 known cases/)
  } finally {
