...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /classification\s+classes\s+methods\s+known cases\s+unknown-case methods\s+share of methods/)
     assert.match(output, /reusable-host\s+43\s+274\s+303\s+0\s+41\.4%/)
     assert.match(output, /fresh-host\s+43\s+254\s+283\s+0\s+38\.4%/)
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 662)
  assert.equal(intentArtifact.totals.cases, 728)
  ...
  assert.equal(hostArtifact.totals.knownCases, 728)
