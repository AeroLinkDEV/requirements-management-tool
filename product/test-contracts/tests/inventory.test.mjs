...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /classification\s+classes\s+methods\s+known cases\s+unknown-case methods\s+share of methods/)
     assert.match(output, /reusable-host\s+40\s+251\s+280\s+0\s+41\.1%/)
     assert.match(output, /fresh-host\s+42\s+235\s+264\s+0\s+38\.5%/)
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 611)
  assert.equal(intentArtifact.totals.cases, 669)
  ...
  assert.equal(hostArtifact.totals.knownCases, 669)
