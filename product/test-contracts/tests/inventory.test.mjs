...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary['reusable-host'], { classes: 46, tests: 303, knownCases: 334, unknownCaseTests: 0 })
...
test('host-classification case totals join exactly to the intent inventory', () => {
  ...
  assert.equal(reusable.tests - reusable.classes, 257)
  assert.equal(reusable.knownCases - reusable.classes, 288)
})
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /reusable-host\s+46\s+303\s+334\s+0\s+35.4%/)
    assert.match(output, /fresh-host\s+47\s+299\s+331\s+0\s+34.9%/)
    assert.match(output, /Remaining reuse headroom:\s+46 classes, 303 methods, 334 known cases/)
  } finally {
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 856)
  assert.equal(intentArtifact.totals.cases, 949)
  ...
  assert.equal(hostArtifact.totals.knownCases, 949)
