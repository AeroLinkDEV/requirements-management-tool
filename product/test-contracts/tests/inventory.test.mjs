...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary['reusable-host'], { classes: 54, tests: 326, knownCases: 367, unknownCaseTests: 0 })
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /reusable-host\s+54\s+326\s+367\s+0\s+35.8%/)
    assert.match(output, /fresh-host\s+54\s+319\s+359\s+0\s+35.0%/)
    assert.match(output, /Remaining reuse headroom:\s+54 classes, 326 methods, 367 known cases/)
  } finally {
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 911)
  assert.equal(intentArtifact.totals.cases, 1022)
  ...
  assert.equal(hostArtifact.totals.knownCases, 1022)
