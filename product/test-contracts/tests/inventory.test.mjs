...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary['fresh-host'], { classes: 44, tests: 274, knownCases: 305, unknownCaseTests: 0 })
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /reusable-host\s+44\s+279\s+310\s+0\s+39.0%/)
    assert.match(output, /fresh-host\s+44\s+274\s+305\s+0\s+38.3%/)
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 715)
  assert.equal(intentArtifact.totals.cases, 798)
  ...
  assert.equal(hostArtifact.totals.knownCases, 798)
