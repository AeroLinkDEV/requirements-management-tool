...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary['fresh-host'], { classes: 60, tests: 366, knownCases: 430, unknownCaseTests: 0 })
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /fresh-host\s+60\s+366\s+430\s+0\s+37.3%/)
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 981)
  assert.equal(intentArtifact.totals.cases, 1121)
  ...
  assert.equal(hostArtifact.totals.knownCases, 1121)
