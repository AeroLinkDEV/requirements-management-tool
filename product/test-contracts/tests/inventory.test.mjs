...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary['fresh-host'], { classes: 57, tests: 342, knownCases: 389, unknownCaseTests: 0 })
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  assert.match(output, /reusable-host\s+56\s+347\s+393\s+0\s+36.3%/)
  assert.match(output, /fresh-host\s+57\s+342\s+389\s+0\s+35.8%/)
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 956)
  assert.equal(intentArtifact.totals.cases, 1079)
  ...
  assert.equal(hostArtifact.totals.knownCases, 1079)
