...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /classification\s+classes\s+methods\s+known cases\s+unknown-case methods\s+share of methods/)
     assert.match(output, /reusable-host\s+42\s+268\s+297\s+0\s+41\.2%/)
     assert.match(output, /fresh-host\s+43\s+248\s+277\s+0\s+38\.2%/)
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 650)
  assert.equal(intentArtifact.totals.cases, 708)
  ...
  assert.equal(hostArtifact.totals.knownCases, 708)
