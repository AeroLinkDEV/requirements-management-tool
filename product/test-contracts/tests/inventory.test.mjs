...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary['reusable-host'], { classes: 40, tests: 251, knownCases: 280, unknownCaseTests: 0 })
  assert.deepEqual(hostArtifact.summary['fresh-host'], { classes: 42, tests: 234, knownCases: 263, unknownCaseTests: 0 })
  assert.deepEqual(hostArtifact.summary.converted, { classes: 21, tests: 119, knownCases: 119, unknownCaseTests: 0 })
...
test('host-classification case totals join exactly to the intent inventory', () => {
  ...
  assert.equal(reusable.tests - reusable.classes, 211)
  assert.equal(reusable.knownCases - reusable.classes, 240)
})
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /classification\s+classes\s+methods\s+known cases\s+unknown-case methods\s+share of methods/)
     assert.match(output, /reusable-host\s+40\s+251\s+280\s+0\s+41\.5%/)
     assert.match(output, /fresh-host\s+42\s+234\s+263\s+0\s+38\.7%/)
     assert.match(output, /Remaining reuse headroom:\s+40 classes, 251 methods, 280 known cases/)
  } finally {
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 605)
  assert.equal(intentArtifact.totals.cases, 663)
  ...
  assert.equal(hostArtifact.totals.knownCases, 663)
