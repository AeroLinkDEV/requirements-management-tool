...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary['reusable-host'], { classes: 31, tests: 195, knownCases: 220, unknownCaseTests: 0 })
  assert.deepEqual(hostArtifact.summary['fresh-host'], { classes: 42, tests: 225, knownCases: 254, unknownCaseTests: 0 })
  assert.deepEqual(hostArtifact.summary.converted, { classes: 20, tests: 106, knownCases: 106, unknownCaseTests: 0 })
...
test('host-classification case totals join exactly to the intent inventory', () => {
  ...
  assert.equal(reusable.tests - reusable.classes, 164)
  assert.equal(reusable.knownCases - reusable.classes, 189)
})
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /classification\s+classes\s+methods\s+known cases\s+unknown-case methods\s+share of methods/)
     assert.match(output, /reusable-host\s+31\s+195\s+220\s+0\s+37\.0%/)
     assert.match(output, /fresh-host\s+42\s+225\s+254\s+0\s+42\.7%/)
    assert.match(output, /Remaining reuse headroom:\s+31 classes, 195 methods, 220 known cases/)
  } finally {
...
test('committed inventories expose per-row case and host evidence', () => {
  assert.equal(intentArtifact.schemaVersion, 'aerolink-api-test-intent/v2')
   assert.equal(intentArtifact.totals.tests, 527)
   assert.equal(intentArtifact.totals.cases, 581)
  ...
  assert.equal(hostArtifact.schemaVersion, 'aerolink-api-host-classification/v3')
   assert.equal(hostArtifact.totals.knownCases, 581)
