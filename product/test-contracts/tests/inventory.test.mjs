...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary['fresh-host'], { classes: 40, tests: 204, knownCases: 229, unknownCaseTests: 0 })
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /reusable-host\s+30\s+186\s+211\s+0\s+42\.0%/)
    assert.match(output, /fresh-host\s+40\s+204\s+229\s+0\s+46\.0%/)
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 443)
  assert.equal(intentArtifact.totals.cases, 493)
  ...
  assert.equal(hostArtifact.totals.knownCases, 493)
