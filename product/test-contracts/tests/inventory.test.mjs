...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary['reusable-host'], { classes: 30, tests: 187, knownCases: 212, unknownCaseTests: 0 })
...
test('host-classification case totals join exactly to the intent inventory', () => {
  ...
  assert.equal(reusable.tests - reusable.classes, 157)
  assert.equal(reusable.knownCases - reusable.classes, 182)
})
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /reusable-host\s+30\s+187\s+212\s+0\s+37\.9%/)
    assert.match(output, /fresh-host\s+41\s+205\s+230\s+0\s+41\.6%/)
    assert.match(output, /Remaining reuse headroom:\s+30 classes, 187 methods, 212 known cases/)
  } finally {
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 493)
  assert.equal(intentArtifact.totals.cases, 543)
  ...
  assert.equal(hostArtifact.totals.knownCases, 543)
