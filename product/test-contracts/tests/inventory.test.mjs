...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary['fresh-host'], { classes: 43, tests: 243, knownCases: 272, unknownCaseTests: 0 })
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /classification\s+classes\s+methods\s+known cases\s+unknown-case methods\s+share of methods/)
     assert.match(output, /reusable-host\s+42\s+267\s+296\s+0\s+41\.7%/)
     assert.match(output, /fresh-host\s+43\s+243\s+272\s+0\s+37\.9%/)
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 641)
  assert.equal(intentArtifact.totals.cases, 699)
  ...
  assert.equal(hostArtifact.totals.knownCases, 699)
