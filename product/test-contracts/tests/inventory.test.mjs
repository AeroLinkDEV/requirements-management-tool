...
test('host-classification case totals join exactly to the intent inventory', () => {
  ...
  assert.equal(reusable.knownCases - reusable.classes, 310)
})
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 904)
  assert.equal(intentArtifact.totals.cases, 1014)
  ...
  assert.equal(hostArtifact.totals.knownCases, 1014)
