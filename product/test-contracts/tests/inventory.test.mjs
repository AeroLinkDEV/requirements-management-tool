...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary['reusable-host'], { classes: 48, tests: 306, knownCases: 341, unknownCaseTests: 0 })
...
test('host-classification case totals join exactly to the intent inventory', () => {
  ...
  assert.equal(reusable.tests - reusable.classes, 258)
  assert.equal(reusable.knownCases - reusable.classes, 293)
})
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /reusable-host\s+48\s+306\s+341\s+0\s+35.6%/)
    assert.match(output, /fresh-host\s+47\s+299\s+331\s+0\s+34.8%/)
    assert.match(output, /Remaining reuse headroom:\s+48 classes, 306 methods, 341 known cases/)
  } finally {
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 859)
  assert.equal(intentArtifact.totals.cases, 956)
  ...
  assert.equal(hostArtifact.totals.knownCases, 956)
