...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary['fresh-host'], { classes: 62, tests: 380, knownCases: 444, unknownCaseTests: 0 })
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /reusable-host\s+57\s+348\s+394\s+0\s+35.0%/)
    assert.match(output, /fresh-host\s+62\s+380\s+444\s+0\s+38.2%/)
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 995)
  assert.equal(intentArtifact.totals.cases, 1135)
  ...
  assert.equal(hostArtifact.totals.knownCases, 1135)
