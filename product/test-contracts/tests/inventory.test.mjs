...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary['reusable-host'], { classes: 42, tests: 264, knownCases: 293, unknownCaseTests: 0 })
...
test('host-classification case totals join exactly to the intent inventory', () => {
  ...
  assert.equal(reusable.tests - reusable.classes, 222)
  assert.equal(reusable.knownCases - reusable.classes, 251)
})
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /classification\s+classes\s+methods\s+known cases\s+unknown-case methods\s+share of methods/)
     assert.match(output, /reusable-host\s+42\s+264\s+293\s+0\s+41\.4%/)
     assert.match(output, /fresh-host\s+43\s+243\s+272\s+0\s+38\.1%/)
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 638)
  assert.equal(intentArtifact.totals.cases, 696)
  ...
  assert.equal(hostArtifact.totals.knownCases, 696)
