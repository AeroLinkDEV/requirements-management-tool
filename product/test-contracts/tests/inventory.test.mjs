...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary['fresh-host'], { classes: 42, tests: 220, knownCases: 249, unknownCaseTests: 0 })
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  assert.match(output, /fresh-host\s+42\s+220\s+249\s+0\s+42\.5%/)
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 518)
  assert.equal(intentArtifact.totals.cases, 572)
  ...
  assert.equal(hostArtifact.totals.knownCases, 572)
