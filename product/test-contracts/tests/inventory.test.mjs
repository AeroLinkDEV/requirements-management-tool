...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary['reusable-host'], { classes: 36, tests: 220, knownCases: 249, unknownCaseTests: 0 })
...
test('host-classification case totals join exactly to the intent inventory', () => {
  ...
  assert.equal(reusable.tests - reusable.classes, 184)
  assert.equal(reusable.knownCases - reusable.classes, 213)
})
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /classification\s+classes\s+methods\s+known cases\s+unknown-case methods\s+share of methods/)
     assert.match(output, /reusable-host\s+36\s+220\s+249\s+0\s+39\.2%/)
     assert.match(output, /fresh-host\s+42\s+231\s+260\s+0\s+41\.2%/)
     assert.match(output, /Remaining reuse headroom:\s+36 classes, 220 methods, 249 known cases/)
  } finally {
...
test('committed inventories expose per-row case and host evidence', () => {
  assert.equal(intentArtifact.schemaVersion, 'aerolink-api-test-intent/v2')
    assert.equal(intentArtifact.totals.tests, 561)
    assert.equal(intentArtifact.totals.cases, 619)
  ...
  assert.equal(hostArtifact.schemaVersion, 'aerolink-api-host-classification/v3')
    assert.equal(hostArtifact.totals.knownCases, 619)
