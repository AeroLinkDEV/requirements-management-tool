...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary['fresh-host'], { classes: 59, tests: 358, knownCases: 413, unknownCaseTests: 0 })
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  assert.match(output, /reusable-host\s+56\s+347\s+393\s+0\s+35.7%/)
  assert.match(output, /fresh-host\s+59\s+358\s+413\s+0\s+36.8%/)
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 972)
  assert.equal(intentArtifact.totals.cases, 1103)
  ...
  assert.equal(hostArtifact.totals.knownCases, 1103)
