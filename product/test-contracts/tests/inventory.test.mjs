...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary['reusable-host'], { classes: 42, tests: 267, knownCases: 296, unknownCaseTests: 0 })
...
test('host-classification case totals join exactly to the intent inventory', () => {
  ...
  assert.equal(reusable.tests - reusable.classes, 225)
  assert.equal(reusable.knownCases - reusable.classes, 254)
})
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /classification\s+classes\s+methods\s+known cases\s+unknown-case methods\s+share of methods/)
     assert.match(output, /reusable-host\s+42\s+267\s+296\s+0\s+42\.2%/)
     assert.match(output, /fresh-host\s+42\s+235\s+264\s+0\s+37\.1%/)
     assert.match(output, /Remaining reuse headroom:\s+42 classes, 267 methods, 296 known cases/)
  } finally {
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 633)
  assert.equal(intentArtifact.totals.cases, 691)
  ...
  assert.equal(hostArtifact.totals.knownCases, 691)
