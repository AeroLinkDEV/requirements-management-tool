...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary['fresh-host'], { classes: 59, tests: 361, knownCases: 420, unknownCaseTests: 0 })
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  assert.match(output, /reusable-host\s+57\s+348\s+394\s+0\s+35.7%/)
  assert.match(output, /fresh-host\s+59\s+361\s+420\s+0\s+37.0%/)
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 976)
  assert.equal(intentArtifact.totals.cases, 1111)
  ...
  assert.equal(hostArtifact.totals.knownCases, 1111)
