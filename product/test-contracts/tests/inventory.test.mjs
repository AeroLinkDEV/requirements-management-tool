...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary['reusable-host'], { classes: 55, tests: 343, knownCases: 389, unknownCaseTests: 0 })
  // Includes the temporary-export expiry boundary while preserving controlled publications.
  assert.deepEqual(hostArtifact.summary['fresh-host'], { classes: 54, tests: 320, knownCases: 360, unknownCaseTests: 0 })
...
test('host-classification case totals join exactly to the intent inventory', () => {
  ...
  assert.equal(reusable.tests - reusable.classes, 288)
  assert.equal(reusable.knownCases - reusable.classes, 334)
})
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /reusable-host\s+55\s+343\s+389\s+0\s+36.9%/)
    assert.match(output, /fresh-host\s+54\s+320\s+360\s+0\s+34.4%/)
    assert.match(output, /Remaining reuse headroom:\s+55 classes, 343 methods, 389 known cases/)
  } finally {
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 930)
  assert.equal(intentArtifact.totals.cases, 1046)
  ...
  assert.equal(hostArtifact.totals.knownCases, 1046)
