...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary.converted, { classes: 27, tests: 166, knownCases: 182, unknownCaseTests: 0 })
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /fresh-host\s+46\s+295\s+327\s+0\s+39.8%/)
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 742)
  assert.equal(intentArtifact.totals.cases, 826)
  ...
  assert.equal(hostArtifact.totals.knownCases, 826)
