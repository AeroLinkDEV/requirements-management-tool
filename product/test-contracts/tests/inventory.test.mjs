...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary.converted, { classes: 22, tests: 123, knownCases: 123, unknownCaseTests: 0 })
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 610)
  assert.equal(intentArtifact.totals.cases, 668)
  ...
  assert.equal(hostArtifact.totals.knownCases, 668)
