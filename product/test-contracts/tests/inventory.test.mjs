...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /reusable-host\s+54\s+327\s+368\s+0\s+35.9%/)
    assert.match(output, /fresh-host\s+54\s+319\s+359\s+0\s+35.0%/)
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 912)
  assert.equal(intentArtifact.totals.cases, 1023)
  ...
  assert.equal(hostArtifact.totals.knownCases, 1023)
