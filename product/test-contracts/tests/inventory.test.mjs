...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary['reusable-host'], { classes: 47, tests: 305, knownCases: 340, unknownCaseTests: 0 })
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /reusable-host\s+47\s+305\s+340\s+0\s+35.5%/)
    assert.match(output, /fresh-host\s+47\s+299\s+331\s+0\s+34.8%/)
    assert.match(output, /Remaining reuse headroom:\s+47 classes, 305 methods, 340 known cases/)
  } finally {
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 858)
  assert.equal(intentArtifact.totals.cases, 955)
  ...
  assert.equal(hostArtifact.totals.knownCases, 955)
