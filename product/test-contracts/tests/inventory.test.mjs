...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary['fresh-host'], { classes: 59, tests: 360, knownCases: 416, unknownCaseTests: 0 })
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  assert.match(output, /fresh-host\s+59\s+360\s+416\s+0\s+36.9%/)
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 975)
  assert.equal(intentArtifact.totals.cases, 1107)
  ...
  assert.equal(hostArtifact.totals.knownCases, 1107)
