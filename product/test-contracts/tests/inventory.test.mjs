...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary['fresh-host'], { classes: 43, tests: 253, knownCases: 282, unknownCaseTests: 0 })
  assert.deepEqual(hostArtifact.summary.converted, { classes: 23, tests: 133, knownCases: 141, unknownCaseTests: 0 })
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /classification\s+classes\s+methods\s+known cases\s+unknown-case methods\s+share of methods/)
     assert.match(output, /reusable-host\s+43\s+274\s+303\s+0\s+41\.5%/)
     assert.match(output, /fresh-host\s+43\s+253\s+282\s+0\s+38\.3%/)
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 661)
  assert.equal(intentArtifact.totals.cases, 727)
  ...
  assert.equal(hostArtifact.totals.knownCases, 727)
