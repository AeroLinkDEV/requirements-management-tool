...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /classification\s+classes\s+methods\s+known cases\s+unknown-case methods\s+share of methods/)
     assert.match(output, /reusable-host\s+40\s+251\s+280\s+0\s+41\.2%/)
     assert.match(output, /fresh-host\s+42\s+234\s+263\s+0\s+38\.4%/)
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 609)
  assert.equal(intentArtifact.totals.cases, 667)
  ...
  assert.equal(hostArtifact.totals.knownCases, 667)
