...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary['reusable-host'], { classes: 40, tests: 252, knownCases: 281, unknownCaseTests: 0 })
...
test('host-classification case totals join exactly to the intent inventory', () => {
  ...
  assert.equal(reusable.tests - reusable.classes, 212)
  assert.equal(reusable.knownCases - reusable.classes, 241)
})
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /classification\s+classes\s+methods\s+known cases\s+unknown-case methods\s+share of methods/)
     assert.match(output, /reusable-host\s+40\s+252\s+281\s+0\s+41\.2%/)
     assert.match(output, /fresh-host\s+42\s+235\s+264\s+0\s+38\.4%/)
     assert.match(output, /Remaining reuse headroom:\s+40 classes, 252 methods, 281 known cases/)
  } finally {
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 612)
  assert.equal(intentArtifact.totals.cases, 670)
  ...
  assert.equal(hostArtifact.totals.knownCases, 670)
