...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary['fresh-host'], { classes: 57, tests: 338, knownCases: 385, unknownCaseTests: 0 })
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /reusable-host\s+56\s+347\s+393\s+0\s+36.4%/)
    assert.match(output, /fresh-host\s+57\s+338\s+385\s+0\s+35.5%/)
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 952)
  assert.equal(intentArtifact.totals.cases, 1075)
  ...
  assert.equal(hostArtifact.totals.knownCases, 1075)
