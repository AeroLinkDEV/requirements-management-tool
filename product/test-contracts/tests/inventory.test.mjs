...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary.converted, { classes: 30, tests: 231, knownCases: 254, unknownCaseTests: 0 })
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /reusable-host\s+44\s+279\s+310\s+0\s+34.6%/)
    assert.match(output, /fresh-host\s+46\s+295\s+327\s+0\s+36.6%/)
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 807)
  assert.equal(intentArtifact.totals.cases, 898)
  ...
  assert.equal(hostArtifact.totals.knownCases, 898)
