...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary['reusable-host'], { classes: 56, tests: 347, knownCases: 393, unknownCaseTests: 0 })
  // Includes GitLab metadata tests with isolated provider configuration and access-revocation races.
  assert.deepEqual(hostArtifact.summary['fresh-host'], { classes: 55, tests: 324, knownCases: 367, unknownCaseTests: 0 })
  assert.deepEqual(hostArtifact.summary.converted, { classes: 33, tests: 256, knownCases: 281, unknownCaseTests: 0 })
  ...
  assert.deepEqual(hostArtifact.summary['migration-candidate'], { classes: 5, tests: 11, knownCases: 16, unknownCaseTests: 0 })
})
...
test('host-classification case totals join exactly to the intent inventory', () => {
  ...
  assert.equal(reusable.tests - reusable.classes, 291)
  assert.equal(reusable.knownCases - reusable.classes, 337)
})
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /reusable-host\s+56\s+347\s+393\s+0\s+37.0%/)
    assert.match(output, /fresh-host\s+55\s+324\s+367\s+0\s+34.5%/)
    assert.match(output, /Remaining reuse headroom:\s+56 classes, 347 methods, 393 known cases/)
  } finally {
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 938)
  assert.equal(intentArtifact.totals.cases, 1057)
  ...
  assert.equal(hostArtifact.totals.knownCases, 1057)
  ...
  for (const cls of ['ReleasedExecutionEvidenceApiTests', 'ReleasedExecutionEvidenceAuthorityMismatchTests', 'ProblemReportPagingApiTests', 'ProductionRoutingTests', 'MainCurrencyTests']) {
