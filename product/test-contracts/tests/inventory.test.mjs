...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary['reusable-host'], { classes: 31, tests: 192, knownCases: 217, unknownCaseTests: 0 })
  assert.deepEqual(hostArtifact.summary['fresh-host'], { classes: 42, tests: 219, knownCases: 248, unknownCaseTests: 0 })
  assert.deepEqual(hostArtifact.summary.converted, { classes: 20, tests: 105, knownCases: 105, unknownCaseTests: 0 })
...
test('host-classification case totals join exactly to the intent inventory', () => {
  ...
  assert.equal(reusable.tests - reusable.classes, 161)
  assert.equal(reusable.knownCases - reusable.classes, 186)
})
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /reusable-host\s+31\s+192\s+217\s+0\s+37\.1%/)
    assert.match(output, /fresh-host\s+42\s+219\s+248\s+0\s+42\.4%/)
    assert.match(output, /Remaining reuse headroom:\s+31 classes, 192 methods, 217 known cases/)
  } finally {
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 517)
  assert.equal(intentArtifact.totals.cases, 571)
  ...
  assert.equal(hostArtifact.totals.knownCases, 571)
