...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /reusable-host\s+30\s+186\s+211\s+0\s+40\.6%/)
    assert.match(output, /fresh-host\s+40\s+203\s+228\s+0\s+44\.3%/)
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 458)
  assert.equal(intentArtifact.totals.cases, 508)
  ...
  assert.equal(hostArtifact.totals.knownCases, 508)
