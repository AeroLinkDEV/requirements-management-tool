...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary['fresh-host'], { classes: 62, tests: 381, knownCases: 445, unknownCaseTests: 0 })
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /fresh-host\s+62\s+381\s+445\s+0\s+38.1%/)
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 999)
  assert.equal(intentArtifact.totals.cases, 1139)
  ...
  assert.equal(hostArtifact.totals.knownCases, 1139)
