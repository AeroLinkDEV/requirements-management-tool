...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary.converted, { classes: 12, tests: 61, knownCases: 61, unknownCaseTests: 0 })
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /reusable-host\s+30\s+186\s+211\s+0\s+41\.2%/)
    assert.match(output, /fresh-host\s+40\s+203\s+228\s+0\s+45\.0%/)
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 451)
  assert.equal(intentArtifact.totals.cases, 501)
  ...
  assert.equal(hostArtifact.totals.knownCases, 501)
