...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary['reusable-host'], { classes: 47, tests: 304, knownCases: 336, unknownCaseTests: 0 })
...
test('host-classification case totals join exactly to the intent inventory', () => {
  ...
  assert.equal(reusable.knownCases - reusable.classes, 289)
})
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /reusable-host\s+47\s+304\s+336\s+0\s+35.5%/)
    assert.match(output, /fresh-host\s+47\s+299\s+331\s+0\s+34.9%/)
    assert.match(output, /Remaining reuse headroom:\s+47 classes, 304 methods, 336 known cases/)
  } finally {
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 857)
  assert.equal(intentArtifact.totals.cases, 951)
  ...
  assert.equal(hostArtifact.totals.knownCases, 951)
