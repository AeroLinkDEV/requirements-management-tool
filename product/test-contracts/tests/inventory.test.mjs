...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary['fresh-host'], { classes: 60, tests: 367, knownCases: 431, unknownCaseTests: 0 })
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /reusable-host\s+57\s+350\s+396\s+0\s+35.5%/)
    assert.match(output, /fresh-host\s+60\s+367\s+431\s+0\s+37.3%/)
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 985)
  assert.equal(intentArtifact.totals.cases, 1125)
  ...
  assert.equal(hostArtifact.totals.knownCases, 1125)
