...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary['reusable-host'], { classes: 52, tests: 322, knownCases: 363, unknownCaseTests: 0 })
...
test('host-classification case totals join exactly to the intent inventory', () => {
  ...
  assert.equal(reusable.knownCases - reusable.classes, 311)
})
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /reusable-host\s+52\s+322\s+363\s+0\s+35.6%/)
    assert.match(output, /fresh-host\s+53\s+317\s+357\s+0\s+35.0%/)
    assert.match(output, /Remaining reuse headroom:\s+52 classes, 322 methods, 363 known cases/)
  } finally {
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 905)
  assert.equal(intentArtifact.totals.cases, 1016)
  ...
  assert.equal(hostArtifact.totals.knownCases, 1016)
