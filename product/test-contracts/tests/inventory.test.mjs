...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary['reusable-host'], { classes: 38, tests: 237, knownCases: 266, unknownCaseTests: 0 })
...
test('host-classification case totals join exactly to the intent inventory', () => {
  ...
  assert.equal(reusable.tests - reusable.classes, 199)
  assert.equal(reusable.knownCases - reusable.classes, 228)
})
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /classification\s+classes\s+methods\s+known cases\s+unknown-case methods\s+share of methods/)
     assert.match(output, /reusable-host\s+38\s+237\s+266\s+0\s+40\.1%/)
     assert.match(output, /fresh-host\s+42\s+234\s+263\s+0\s+39\.6%/)
     assert.match(output, /Remaining reuse headroom:\s+38 classes, 237 methods, 266 known cases/)
  } finally {
...
test('committed inventories expose per-row case and host evidence', () => {
  assert.equal(intentArtifact.schemaVersion, 'aerolink-api-test-intent/v2')
    assert.equal(intentArtifact.totals.tests, 591)
    assert.equal(intentArtifact.totals.cases, 649)
  ...
  assert.equal(hostArtifact.schemaVersion, 'aerolink-api-host-classification/v3')
    assert.equal(hostArtifact.totals.knownCases, 649)
