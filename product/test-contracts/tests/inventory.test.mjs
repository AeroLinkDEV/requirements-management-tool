...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /classification\s+classes\s+methods\s+known cases\s+unknown-case methods\s+share of methods/)
     assert.match(output, /reusable-host\s+42\s+268\s+297\s+0\s+41\.7%/)
     assert.match(output, /fresh-host\s+43\s+244\s+273\s+0\s+37\.9%/)
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 643)
  assert.equal(intentArtifact.totals.cases, 701)
  ...
  assert.equal(hostArtifact.totals.knownCases, 701)
