...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary['reusable-host'], { classes: 32, tests: 198, knownCases: 223, unknownCaseTests: 0 })
...
test('host-classification case totals join exactly to the intent inventory', () => {
  ...
  assert.equal(reusable.tests - reusable.classes, 166)
  assert.equal(reusable.knownCases - reusable.classes, 191)
})
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /reusable-host\s+32\s+198\s+223\s+0\s+39\.3%/)
    assert.match(output, /fresh-host\s+41\s+205\s+230\s+0\s+40\.7%/)
    assert.match(output, /Remaining reuse headroom:\s+32 classes, 198 methods, 223 known cases/)
  } finally {
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 504)
  assert.equal(intentArtifact.totals.cases, 554)
  ...
  assert.equal(hostArtifact.totals.knownCases, 554)
