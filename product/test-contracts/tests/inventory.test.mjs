...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary['fresh-host'], { classes: 44, tests: 268, knownCases: 297, unknownCaseTests: 0 })
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /reusable-host\s+44\s+279\s+310\s+0\s+39.4%/)
    assert.match(output, /fresh-host\s+44\s+268\s+297\s+0\s+37.8%/)
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 709)
  assert.equal(intentArtifact.totals.cases, 790)
  ...
  assert.equal(hostArtifact.totals.knownCases, 790)
