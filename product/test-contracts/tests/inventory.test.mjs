...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary['reusable-host'], { classes: 31, tests: 193, knownCases: 218, unknownCaseTests: 0 })
...
test('host-classification case totals join exactly to the intent inventory', () => {
  ...
  assert.equal(reusable.tests - reusable.classes, 162)
  assert.equal(reusable.knownCases - reusable.classes, 187)
})
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /reusable-host\s+31\s+193\s+218\s+0\s+37\.0%/)
    assert.match(output, /fresh-host\s+42\s+222\s+251\s+0\s+42\.5%/)
    assert.match(output, /Remaining reuse headroom:\s+31 classes, 193 methods, 218 known cases/)
  } finally {
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 522)
  assert.equal(intentArtifact.totals.cases, 576)
  ...
  assert.equal(hostArtifact.totals.knownCases, 576)
