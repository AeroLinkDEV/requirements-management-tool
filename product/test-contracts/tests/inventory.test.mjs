...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary.converted, { classes: 16, tests: 83, knownCases: 83, unknownCaseTests: 0 })
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /reusable-host\s+30\s+186\s+211\s+0\s+39\.2%/)
    assert.match(output, /fresh-host\s+41\s+204\s+229\s+0\s+43\.0%/)
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 474)
  assert.equal(intentArtifact.totals.cases, 524)
  ...
  assert.equal(hostArtifact.totals.knownCases, 524)
