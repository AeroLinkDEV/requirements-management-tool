...
test('reviewed #563 holds keep every unsafe reusable class out of reuse headroom', () => {
  ...
  assert.deepEqual(hostArtifact.summary.converted, { classes: 27, tests: 167, knownCases: 183, unknownCaseTests: 0 })
...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /reusable-host\s+44\s+279\s+310\s+0\s+37.6%/)
    assert.match(output, /fresh-host\s+46\s+295\s+327\s+0\s+39.7%/)
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 743)
  assert.equal(intentArtifact.totals.cases, 827)
  ...
  assert.equal(hostArtifact.totals.knownCases, 827)
