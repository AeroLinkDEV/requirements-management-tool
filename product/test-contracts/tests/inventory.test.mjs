...
test('host classification CLI distinguishes known cases from unknown-case methods', () => {
  ...
  try {
    ...
    assert.match(output, /reusable-host\s+44\s+278\s+309\s+0\s+39.4%/)
    assert.match(output, /fresh-host\s+44\s+266\s+295\s+0\s+37.7%/)
...
test('committed inventories expose per-row case and host evidence', () => {
  ...
  assert.equal(intentArtifact.totals.tests, 706)
  assert.equal(intentArtifact.totals.cases, 787)
  ...
  assert.equal(hostArtifact.totals.knownCases, 787)
