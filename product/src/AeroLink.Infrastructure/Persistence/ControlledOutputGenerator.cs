    ...
    ILadderPolicy? policy = null, IProjectLadderPolicyResolver? policyResolver = null)
{
    ...
    private async Task<List<PublicationRecord>> RequirementPublicationRows(Guid baselineId, Guid projectId, RequirementLevel level, CancellationToken ct)
    {
        ...
        static string ExactParentIds(string json)
        {
            try
            {
                var ids = System.Text.Json.JsonSerializer.Deserialize<List<Guid>>(string.IsNullOrWhiteSpace(json) ? "[]" : json)
                    ?? [];
                return string.Join("; ", ids.Distinct().OrderBy(x => x).Select(x => x.ToString()));
            }
            catch (System.Text.Json.JsonException)
            {
                return "Invalid exact parent identity";
            }
        }
    }
        ...
        DateTimeOffset generatedAt, ILadderPolicy ladderPolicy, CancellationToken ct)
    {
        ...
        var sourceIds=sources.Select(x=>x.Id).ToList();var baselineRevisionIds=await db.BaselineRequirements.AsNoTracking().Where(x=>x.BaselineId==baselineId).Select(x=>x.RevisionId).ToHashSetAsync(ct);var links=(await db.RequirementTraces.AsNoTracking().Where(x=>sourceIds.Contains(x.SourceRevisionId)&&baselineRevisionIds.Contains(x.TargetRevisionId)&&(x.ExactLinkSuspectLifecycleId==null||db.ExactLinkSuspectLifecycles.Any(lifecycle=>lifecycle.Id==x.ExactLinkSuspectLifecycleId&&lifecycle.LinkKind==ExactLinkKind.RequirementTrace&&lifecycle.State==ExactLinkLifecycleState.Closed))).ToListAsync(ct)).Where(x=>x.CreatedAt<=generatedAt).ToList();var targetIds=links.Select(x=>x.TargetRevisionId).Distinct().ToList();
    ...
    private async Task<List<OutputRow>> ProcedureRows(Guid projectId, TestProcedureLevel level, CancellationToken ct) => await (from procedure in db.TestProcedures.AsNoTracking().Where(x => x.ProjectId == projectId && x.Level == level && (x.Level == TestProcedureLevel.System || x.ArtifactKind == VerificationArtifactKind.Case))
