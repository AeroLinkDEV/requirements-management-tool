    ...
    AeroLinkDbContext db, ControlledOutputGenerator generator, EvidenceFileStore files)
{
    ...
    public async Task EnsureCompletedAsync(CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            // AeroLink deliberately supports documents created before exact verification manifests existed,
            // and reconstructs those from the document's own generation-time compatibility snapshot. Do not
            // manufacture a baseline materialization marker/hash merely to migrate such a document.
            ...
            // the no-tracking lookup below; otherwise an exact document basis could still use the pre-rename
            // hash even though the final database row claims the new one.
        ...
        ControlledDocument document, DateTimeOffset now, CancellationToken ct)
    {
        var baseline = await db.CandidateBaselines.AsNoTracking()
            ...
            .Select(x => new { x.Id, x.TestProceduresMaterializedAt, x.TestProceduresHash })
        ...
        if (snapshot.Rows.Count != document.ArtifactCount)
            throw new InvalidOperationException(
                $"Software Case migration cannot reconstruct legacy document {document.Id} on baseline {baseline.Id}: generation-time compatibility snapshot contains {snapshot.Rows.Count} records but the controlled document records {document.ArtifactCount}.");

        var revisionIds = snapshot.Rows.Select(x => x.RevisionId).Distinct().ToList();
        if (revisionIds.Count != snapshot.Rows.Count)
            throw new InvalidOperationException(
                $"Software Case migration cannot reconstruct legacy document {document.Id} on baseline {baseline.Id}: the generation-time compatibility snapshot contains duplicate revision identities.");
        var owners = await db.TestProcedureRevisions.AsNoTracking()
            .Where(x => revisionIds.Contains(x.Id))
            .Select(x => new { x.Id, x.ProcedureId })
            .ToListAsync(ct);
        if (owners.Count != revisionIds.Count)
            throw new InvalidOperationException(
                $"Software Case migration cannot reconstruct legacy document {document.Id} on baseline {baseline.Id}: one or more compatibility-snapshot revisions no longer exist.");
        var procedureIdByRevision = owners.ToDictionary(x => x.Id, x => x.ProcedureId);
        var entries = snapshot.Rows.Select(row => new TestProcedureManifestEntry(
            procedureIdByRevision[row.RevisionId], row.RevisionId, row.BaseNumber, row.Revision)).ToList();
        var compatibilitySnapshotHash = TestProcedureManifest.Hash(entries);

        db.SecurityAuditEvents.Add(new SecurityAuditEvent(
            "VerificationIdentityMigration.LegacyDocumentBasisReconstructed", Actor,
