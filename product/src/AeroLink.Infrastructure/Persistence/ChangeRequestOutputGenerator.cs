...
public sealed class ChangeRequestOutputGenerator(AeroLinkDbContext db)
{
    public async Task<GeneratedOutput?> GenerateAsync(Guid changeRequestId, string format, CancellationToken ct)
    {
        ...
        var audit = scr.AuditEvents.OrderBy(x => x.OccurredAt).Select((x, i) => new PublicationRecord((i + 1).ToString("D3"), x.EventType, "", CanonicalAuditText(x.Detail), new[] { ("Actor", Person(x.ActorId)), ("Occurred", x.OccurredAt.UtcDateTime.ToString("yyyy-MM-dd HH:mm 'UTC'")) })).ToList();
        ...
        var publicationTitle = scr.Type switch
        {
            ChangeRequestType.System => "System Change Request",
