...
public sealed class AeroLinkDbContext(DbContextOptions<AeroLinkDbContext> options) : DbContext(options)
{
    ...
    public DbSet<RollbackCleanupFailureEvidence> RollbackCleanupFailureEvidences =>
        Set<RollbackCleanupFailureEvidence>();
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<ProjectLadderAllowedUpstream>(b =>
        {
            b.ToTable("project_ladder_allowed_upstreams", t => t.HasCheckConstraint("CK_project_ladder_upstream_not_self", "\"ParentStepId\" <> \"ChildStepId\""));
            b.ToTable("project_ladder_allowed_upstreams", t => t.HasCheckConstraint("CK_project_ladder_upstream_version", "\"Version\" > 0"));
            b.HasKey(x => x.Id); b.Property(x => x.Version).IsConcurrencyToken();
            b.HasIndex(x => new { x.ConfigurationId, x.ParentStepId, x.ChildStepId }).IsUnique();
            b.HasOne<ProjectLadderStep>().WithMany().HasForeignKey(nameof(ProjectLadderAllowedUpstream.ConfigurationId), nameof(ProjectLadderAllowedUpstream.ProjectId), nameof(ProjectLadderAllowedUpstream.ParentStepId))
                .HasPrincipalKey(nameof(ProjectLadderStep.ConfigurationId), nameof(ProjectLadderStep.ProjectId), nameof(ProjectLadderStep.Id)).OnDelete(DeleteBehavior.Cascade);
            b.HasOne<ProjectLadderStep>().WithMany().HasForeignKey(nameof(ProjectLadderAllowedUpstream.ConfigurationId), nameof(ProjectLadderAllowedUpstream.ProjectId), nameof(ProjectLadderAllowedUpstream.ChildStepId))
                .HasPrincipalKey(nameof(ProjectLadderStep.ConfigurationId), nameof(ProjectLadderStep.ProjectId), nameof(ProjectLadderStep.Id)).OnDelete(DeleteBehavior.Cascade);
        });
        ...
        modelBuilder.Entity<BaselineEvent>(b =>
        {
            ...
            // Alternate key used by the composite provenance FK, proving a provenance row's event belongs to
            // the same baseline.
            b.HasIndex(x => new { x.BaselineId, x.Id }).IsUnique();
        });
        modelBuilder.Entity<BaselineExecutionCutoverProvenance>(b =>
        {
            ...
            b.HasOne<CandidateBaseline>().WithMany().HasForeignKey(x => x.BaselineId)
                .OnDelete(DeleteBehavior.Restrict);
            // Composite FK through the baseline_events alternate key: EventId must exist AND belong to the
            // same baseline. PostgreSQL triggers additionally require the event to be the
            // ExecutionCutoverManifestMigrated summary; immutability is enforced at the database boundary.
            b.HasOne<BaselineEvent>().WithMany()
                .HasForeignKey(x => new { x.BaselineId, x.EventId })
                .HasPrincipalKey(entry => new { entry.BaselineId, entry.Id })
                .OnDelete(DeleteBehavior.Restrict);
        });
        ...
        modelBuilder.Entity<TestProcedureRevision>(b =>
        {
            ...
            b.Property(x => x.EnvironmentSetup).HasMaxLength(8000).IsRequired(); b.Property(x => x.TestData).HasMaxLength(8000).IsRequired();
            b.Property(x => x.OrderedSteps).HasMaxLength(16000).IsRequired(); b.Property(x => x.ExpectedObservations).HasMaxLength(8000).IsRequired();
            b.Property(x => x.Cleanup).HasMaxLength(8000).IsRequired(); b.Property(x => x.ToolingAutomation).HasMaxLength(8000).IsRequired();
            b.Property(x => x.ParentKind).HasConversion<string>().HasMaxLength(30).IsRequired(); b.Property(x => x.DerivedRationale).HasMaxLength(4000).IsRequired();
            b.Property(x => x.RetirementRationale).HasMaxLength(4000).IsRequired();
            ...
            // Alternate key used by the composite migration-source FK, proving a source's generated revision
            // belongs to its generated artifact.
            b.HasIndex(x => new { x.ProcedureId, x.Id }).IsUnique();
        ...
        modelBuilder.Entity<TestCaseProcedureLink>(b =>
        {
        ...
        modelBuilder.Entity<TestProcedureMigrationSource>(b =>
        {
            ...
            b.HasOne<ProjectRecord>().WithMany().HasForeignKey(x => x.ProjectId).OnDelete(DeleteBehavior.Restrict);
            b.HasOne<TestProcedureRevision>().WithMany()
                .HasForeignKey(x => x.SourceCaseRevisionId).OnDelete(DeleteBehavior.Restrict);
            b.HasOne<TestProcedure>().WithMany()
                .HasForeignKey(x => x.GeneratedProcedureArtifactId).OnDelete(DeleteBehavior.Restrict);
            b.HasOne<TestProcedureRevision>().WithMany()
                .HasForeignKey(x => x.GeneratedProcedureRevisionId).OnDelete(DeleteBehavior.Restrict);
            // Composite FK through test_procedure_revisions (ProcedureId, Id): the generated revision must
            // belong to the generated artifact. PostgreSQL triggers additionally enforce artifact kind,
            // project ownership, and source-Case semantics; immutability is enforced at the database boundary.
            b.HasOne<TestProcedureRevision>().WithMany()
                .HasForeignKey(x => new { x.GeneratedProcedureArtifactId, x.GeneratedProcedureRevisionId })
                .HasPrincipalKey(revision => new { revision.ProcedureId, revision.Id })
                .OnDelete(DeleteBehavior.Restrict);
        });
        modelBuilder.Entity<RollbackCleanupFailureEvidence>(b =>
        {
            b.ToTable("rollback_cleanup_failure_evidence"); b.HasKey(x => x.Id);
            b.Property(x => x.CanonicalAggregateHash).HasMaxLength(64).IsRequired();
            b.Property(x => x.Content).HasMaxLength(1500).IsRequired();
            b.HasIndex(x => new { x.OperationId, x.Sequence }).IsUnique();
        });
        ...
        modelBuilder.Entity<ProblemReport>(b =>
        {
            ...
            // The authored companion to every narrative field, sized like ProblemRich rather than like the
            // plain column: structure costs more characters than the text it carries.
            b.Property(x => x.AnalysisRich).HasMaxLength(32000); b.Property(x => x.RootCauseRich).HasMaxLength(32000); b.Property(x => x.EffectsRich).HasMaxLength(32000); b.Property(x => x.ContainmentRich).HasMaxLength(32000); b.Property(x => x.CorrectiveActionRich).HasMaxLength(32000); b.Property(x => x.SystemAircraftImpactRich).HasMaxLength(32000); b.Property(x => x.WorkaroundRich).HasMaxLength(32000);
            ...
            // By name, so adding a category later costs nothing in the database. Both are nullable: a Draft
            // nobody has classified yet stores null rather than a sentinel that would read as an answer.
            b.Property(x => x.Category).HasConversion<string>().HasMaxLength(40);
            b.Property(x => x.CategoryProvenance).HasConversion<string>().HasMaxLength(30);
        ...
        {
            b.ToTable("problem_report_revisions"); b.HasKey(x => x.Id); b.Property(x => x.EventType).HasMaxLength(80).IsRequired(); b.Property(x => x.Actor).HasMaxLength(100).IsRequired(); b.Property(x => x.ActorDisplayName).HasMaxLength(200); b.Property(x => x.SnapshotHash).HasMaxLength(64).IsRequired(); b.Property(x => x.SnapshotJson).IsRequired(); b.Property(x => x.SnapshotSchemaVersion).IsRequired(); b.Property(x => x.Detail).HasMaxLength(2000).IsRequired(); b.Property(x => x.FromState).HasMaxLength(40).IsRequired(); b.Property(x => x.ToState).HasMaxLength(40).IsRequired(); b.Property(x => x.Rationale).HasMaxLength(4000).IsRequired(); b.Property(x => x.EvidenceJson); b.Property(x => x.EventSchemaVersion).IsRequired();
    ...
    private async Task<int> SaveChangesCoreAsync(bool acceptAllChangesOnSuccess, CancellationToken cancellationToken)
    {
        ...
        await ValidateVerificationIntegrityAsync(cancellationToken);
    ...
    {
        var trackedEvents = ChangeTracker.Entries<BaselineEvent>()
            .Where(x => x.State != EntityState.Deleted)
            .ToDictionary(x => x.Entity.Id, x => x.Entity);
        var trackedProcedures = ChangeTracker.Entries<TestProcedure>()
            .Where(x => x.State != EntityState.Deleted)
            .ToDictionary(x => x.Entity.Id, x => x.Entity);
        ...
        foreach (var entry in sources)
        {
            ...
            {
                var persisted = await (from revision in TestProcedureRevisions.AsNoTracking()
            ...
            if (generated is null)
                generated = await TestProcedures.AsNoTracking()
            ...
            if (generatedRevision is null)
                generatedRevision = await TestProcedureRevisions.AsNoTracking()
    ...
    private async Task ValidateRequirementParentSelectionsAsync(CancellationToken ct)
    {
        ...
        var trackedParentRevisions = ChangeTracker.Entries<RequirementRevision>()
        ...
        var parentArtifacts = await Requirements.AsNoTracking()
        ...
        var activeParentIds = (await RequirementRevisions.AsNoTracking()
    ...
    {
        _ = ct;
        var addedSoftwareProcedures = ChangeTracker.Entries<TestProcedure>()
            .Where(x => x.State == EntityState.Added
                && x.Entity.ArtifactKind == VerificationArtifactKind.Procedure
        ...
        var initialRevisionOwners = ChangeTracker.Entries<TestProcedureRevision>()
            .Where(x => x.State == EntityState.Added
    ...
    private async Task RefuseCrossLevelCoverageAsync(CancellationToken ct)
    {
        ...
        var trackedProcedureOwners = ChangeTracker.Entries<TestProcedure>()
            .Where(x => x.State != EntityState.Deleted)
            .ToDictionary(x => x.Entity.Id, x => x.Entity);
        var changedProcedureRevisions = ChangeTracker.Entries<TestProcedureRevision>()
        ...
        var missingProcedureOwnerIds = changedProcedureRevisions
            ...
            .ToList();
        var persistedProcedureOwners = await TestProcedures.AsNoTracking()
        ...
        var requirementLevels = await (from revision in RequirementRevisions.AsNoTracking()
            ...
            .ToDictionaryAsync(x => x.RevisionId, ct);
        var trackedRequirementArtifacts = ChangeTracker.Entries<RequirementArtifact>()
            .Where(x => x.State != EntityState.Deleted)
            .ToDictionary(x => x.Entity.Id, x => x.Entity);
        var changedRequirementRevisions = ChangeTracker.Entries<RequirementRevision>()
        ...
        var missingRequirementArtifactIds = changedRequirementRevisions
            ...
            .ToList();
        var persistedRequirementArtifacts = await Requirements.AsNoTracking()
    ...
    private async Task ValidateProcedureParentsAsync(CancellationToken ct)
    {
        var revisionIds = ChangeTracker.Entries<TestProcedureRevision>()
            ...
            .Select(x => x.Entity.Id)
            .Concat(ChangeTracker.Entries<TestCaseProcedureLink>()
                .Where(x => x.State != EntityState.Unchanged)
        ...
        foreach (var tracked in ChangeTracker.Entries<TestProcedure>().Where(x => x.State != EntityState.Deleted
    ...
    private async Task ValidateCaseSystemParentSelectionsAsync(CancellationToken ct)
    {
        ...
        var revisionEntryById = ChangeTracker.Entries<TestProcedureRevision>()
