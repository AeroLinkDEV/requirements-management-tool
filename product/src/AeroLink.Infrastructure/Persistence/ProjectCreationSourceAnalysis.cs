...
public static class ProjectCreationSourceParser
{
    ...
    public static ProjectCreationSourceAnalysis Analyse(Stream source, string fileName)
    {
        ...
        var result = extension switch
        {
            ...
            _ => throw new InvalidOperationException("Select a ReqIF (.reqif), CSV (.csv), or Excel (.xlsx) source file.")
        };
    ...
    private static ProjectCreationSourceAnalysis AnalyseReqIf(Stream stream)
    {
        ...
        var document = ReqIfExchangeService.ReadSourceXml(stream);
        var objects = new List<ProjectCreationSourceObject>();
        ...
        var definitionNames = definitions.ToDictionary(x => Attribute(x, "IDENTIFIER"), x => Attribute(x, "LONG-NAME"));
        ...
        foreach (var item in document.Descendants().Where(x => x.Name.LocalName == "SPEC-OBJECT"))
        {
            ...
            var values = Values(item, definitionNames);
            var sourceIdentifier = Pick(values, "AeroLink.Identifier", "Identifier", "ReqIF.ForeignID", "ID");
            ...
            var kind = Pick(values, "AeroLink.Kind");
        ...
        foreach (var relation in document.Descendants().Where(x => x.Name.LocalName == "SPEC-RELATION"))
        {
            ...
            relations.Add(new(key, Reference("SOURCE"), Reference("TARGET"), Reference("TYPE"), Values(relation, definitionNames)));
    ...
    private static Dictionary<string, string> Values(XElement item, IReadOnlyDictionary<string, string> names)
    {
        ...
        foreach (var value in item.Elements().Where(x => x.Name.LocalName == "VALUES").Elements())
        {
            ...
            var name = names.GetValueOrDefault(definition, definition);
            if (name.Length == 0) name = definition;
            if (name.Length == 0 || output.ContainsKey(name)) throw new InvalidOperationException("ReqIF values need distinct attributable attribute definitions.");
