...
static class ApiMap
{
    ...
    public static object ChangeRequestDetail(SystemChangeRequest x, IReadOnlyList<object>? contention = null) => new
    {
        ...
        noUpstream = x.NoUpstreamRationale is null ? null : new { rationale = x.NoUpstreamRationale, actor = x.NoUpstreamStatedBy, statedAt = x.NoUpstreamStatedAt },
        inheritedUpstream = x.InheritedUpstreamContextJson is null ? null : new { x.InheritedFromChangeRequestId, x.InheritedAt, x.InheritedUpstreamContextJson, affirmed = x.UpstreamAnswerAffirmed, affirmedBy = x.UpstreamAnswerAffirmedBy, affirmedAt = x.UpstreamAnswerAffirmedAt },
        upstream = x.UpstreamLinks.OrderBy(l => l.UpstreamDisplayNumber).Select(l => new { l.Id, l.UpstreamChangeRequestId, l.UpstreamDisplayNumber, l.UpstreamBuildId, l.UpstreamBuildVersion, l.Rationale, actor = l.ActorId, statedAt = l.StatedAt }),
        upstreamHistory = x.UpstreamHistory.OrderByDescending(h => h.OccurredAt).Select(h => new { h.Id, h.Action, h.UpstreamLinkId, h.UpstreamChangeRequestId, h.UpstreamDisplayNumber, h.UpstreamBuildId, h.UpstreamBuildVersion, h.Rationale, actor = h.ActorId, occurredAt = h.OccurredAt }),
        reviewCycles = x.ReviewCycles.OrderBy(c => c.Sequence).Select(c => new { c.Id, c.Sequence, mode=c.Mode.ToString(), state = c.State.ToString(), c.WorkflowId, c.WorkflowLogicalId, c.WorkflowName, c.WorkflowVersion, c.SnapshotHash, c.SnapshotContractVersion, c.SnapshotJson, c.StartedAt, c.CompletedAt, c.ClosureReason, steps = c.Steps.OrderBy(s => s.Position).Select(s => new { s.Id, s.Position, s.ApproverId, s.ApproverName, s.Authority, stageKind = s.StageKind.ToString(), authoritySource = s.AuthoritySource?.ToString(), s.AuthoritySourceId, s.StageName, s.Rationale, state = s.State.ToString(), s.DecidedAt }) }),
