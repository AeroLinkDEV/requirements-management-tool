...
public static class EditSessionEndpoints
{
    public static void MapEditSessionEndpoints(this WebApplication app)
    {
        ...
        app.MapGet("/api/enterprise-hardening/jobs/{id:guid}/download",async(Guid id,HttpContext http,AeroLinkDbContext db,EvidenceFileStore store,CancellationToken ct)=>{var job=await db.EnterpriseOperationJobs.AsNoTracking().SingleOrDefaultAsync(x=>x.Id==id,ct);if(job is null)return Results.NotFound();if(!await http.HasProjectAccessAsync(db,job.ProjectId,ct))return Results.Forbid();if(job.State!=EnterpriseJobState.Completed)return Results.BadRequest(new{error="The output is not complete."});using var result=JsonDocument.Parse(job.ResultJson);if(job.JobType=="BackgroundRepositoryExport"&&result.RootElement.TryGetProperty("ExpiresAt",out var expiry)&&expiry.TryGetDateTimeOffset(out var expiresAt)&&expiresAt<=DateTimeOffset.UtcNow)return Results.Json(new{error="This temporary export has expired. Generate a new export.",code="export_expired"},statusCode:410);if(!result.RootElement.TryGetProperty("StorageKey",out var storage)&&!result.RootElement.TryGetProperty("storageKey",out storage))return Results.BadRequest(new{error="This job has no downloadable output."});var fileName=result.RootElement.TryGetProperty("OriginalFileName",out var name)||result.RootElement.TryGetProperty("originalFileName",out name)?name.GetString():$"aerolink-export-{id:N}.bin";var contentType=result.RootElement.TryGetProperty("ContentType",out var type)||result.RootElement.TryGetProperty("contentType",out type)?type.GetString():"application/octet-stream";return Results.File(store.OpenRead(storage.GetString()!),contentType??"application/octet-stream",fileName,enableRangeProcessing:true);});
