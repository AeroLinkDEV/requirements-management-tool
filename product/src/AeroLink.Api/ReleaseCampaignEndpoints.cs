...
public static class ReleaseCampaignEndpoints
{
    public static void MapReleaseCampaignEndpoints(this WebApplication app)
    {
        ...
        app.MapGet("/api/documents/{id:guid}/download", async (Guid id, string? format, HttpContext http, AeroLinkDbContext db, ControlledOutputGenerator generator, EvidenceFileStore store, CancellationToken ct) =>
        {
            ...
            var normalized = (format ?? "docx").Trim().ToLowerInvariant();
            // Documents created after artifact freezing serve their exact stored bytes. Older records have no
            // artifact and fall back to regeneration, which the manifest explicitly labels as legacy.
            var artifact = await db.ControlledDocumentArtifacts.AsNoTracking()
                .SingleOrDefaultAsync(x => x.DocumentId == id && x.Format == normalized, ct);
            if (artifact is not null)
                return Results.File(store.OpenRead(artifact.StorageKey), artifact.ContentType, artifact.OriginalFileName, enableRangeProcessing: true);
            var output = await generator.GenerateAsync(id, normalized, ct); return output is null ? Results.NotFound() : Results.File(output.Content, output.ContentType, output.FileName);
        });
        ...
        {
            var document=await db.ControlledDocuments.AsNoTracking().SingleOrDefaultAsync(x=>x.Id==id,ct);if(document is null)return Results.NotFound();if(!await http.HasProjectAccessAsync(db,document.ProjectId,ct))return Results.Forbid();var baseline=await db.CandidateBaselines.AsNoTracking().SingleAsync(x=>x.Id==document.BaselineId,ct);var release=await db.Releases.AsNoTracking().SingleAsync(x=>x.Id==document.ReleaseId,ct);var approvalRows=await(from approval in db.ReleaseApprovals.AsNoTracking() join campaign in db.ReleaseCampaigns.AsNoTracking() on approval.CampaignId equals campaign.Id where campaign.ProjectId==document.ProjectId&&campaign.ReleaseId==document.ReleaseId&&campaign.BaselineId==document.BaselineId&&approval.ApprovedAt!=null select new{campaignId=campaign.Id,approval.ApproverId,approval.ApproverName,state=approval.State.ToString(),approval.ApprovedAt}).ToListAsync(ct);var approvals=approvalRows.Where(x=>x.ApprovedAt<=document.GeneratedAt).OrderBy(x=>x.ApprovedAt).ToList();var artifacts=await db.ControlledDocumentArtifacts.AsNoTracking().Where(x=>x.DocumentId==id).OrderBy(x=>x.Format).Select(x=>new{x.Format,x.Sha256,x.Size,x.RenderedAt,x.OriginalFileName}).ToListAsync(ct);object reproducibility=artifacts.Count>0?new{renderer="AeroLink professional publication renderer",basis="frozen artifact",deterministic=true}:new{renderer="AeroLink professional publication renderer",basis="legacy on-demand regeneration; bytes are not frozen",deterministic=false,contentHash=document.ContentHash};return Results.Ok(new{format="AeroLink controlled-publication-manifest/v1",document=new{document.Id,document.DocumentNumber,document.Revision,document.Title,type=document.Type.ToString(),document.ContentHash,document.ArtifactCount,document.GeneratedAt},source=new{baseline=baseline.DisplayNumber,baseline.ContentHash,baseline.RequirementsHash,release=release.Version},approvalEvidence=approvals,artifacts,reproducibility});
        });
        ...
        app.MapPost("/api/release-campaigns/{id:guid}/review", async (Guid id, StartReleaseReviewRequest request, HttpContext http, AeroLinkDbContext db, IdentityService identity, ReleaseReadinessService readiness, ReleaseExecutionService execution, CancellationToken ct) =>
        {
            ...
            try
            {
                ...
                campaign.BeginReleaseReview(freshActor.UserName, requested.Select(userName=>{var person=known.Single(x=>x.UserName==userName);return(person.UserName,person.DisplayName);}).ToList(),manifestHash,DateTimeOffset.UtcNow);
