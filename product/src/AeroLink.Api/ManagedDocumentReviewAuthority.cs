...
internal static class ManagedDocumentReviewAuthority
{
    ...
    public const int LegacyPolicyVersion = 1;
    ...
    public static Task<ManagedDocumentAuthorityEvidence?> ResolveTechnicalAsync(AeroLinkDbContext db,
        ...
        ResolveV2Async(db, programId, account, "TechnicalDocumentReview", Technical, now, ct);

    public static Task<ManagedDocumentAuthorityEvidence?> ResolveFinalAsync(AeroLinkDbContext db,
        Guid programId, UserAccount account, DateTimeOffset now, CancellationToken ct) =>
        ResolveV2Async(db, programId, account, "DocumentReleaseAuthorization", Final, now, ct);

    /// <summary>
    /// Historical v1 interpretation retained so policy-version evidence remains explainable. V1 predated
    /// Project Leadership and treated every accepted role as membership authority. Existing review steps are
    /// frozen at assignment and are never rewritten; these resolvers make the meaning of their version an
    /// explicit, testable contract while all new assignments use v2 above.
    /// </summary>
    internal static Task<ManagedDocumentAuthorityEvidence?> ResolveTechnicalV1Async(AeroLinkDbContext db,
        Guid programId, UserAccount account, DateTimeOffset now, CancellationToken ct) =>
        ResolveV1Async(db, programId, account, "TechnicalDocumentReview", Technical, now, ct);

    internal static Task<ManagedDocumentAuthorityEvidence?> ResolveFinalV1Async(AeroLinkDbContext db,
        Guid programId, UserAccount account, DateTimeOffset now, CancellationToken ct) =>
        ResolveV1Async(db, programId, account, "DocumentReleaseAuthorization", Final, now, ct);

    private static async Task<ManagedDocumentAuthorityEvidence?> ResolveV2Async(AeroLinkDbContext db,
    ...
    private static async Task<ManagedDocumentAuthorityEvidence?> ResolveV1Async(AeroLinkDbContext db,
        Guid programId, UserAccount account, string required, IReadOnlyList<ProgramRole> accepted,
        DateTimeOffset now, CancellationToken ct)
    {
        if (account.State != AccountState.Active) return null;
        ...
        var adminDelegation = delegations.FirstOrDefault(x => x.Role == ProgramRole.Administrator
                                                               && x.StartsAt <= now && x.EndsAt > now);
        if (adminDelegation is not null)
            return new(required, ProgramRole.Administrator, "AdministratorSubstitution", adminDelegation.Id);
        ...
        {
            var backup = backups.FirstOrDefault(x => x.Role == role);
            if (backup is not null && direct.Count > 0)
                return new(required, role, "StandingBackup", backup.Id);
        }
        return null;
    }
}
