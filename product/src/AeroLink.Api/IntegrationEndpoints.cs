...
public static class IntegrationEndpoints
{
    ...
    private static async Task<IResult> ConsumeOslcRequirementAsync(OslcConsumeRequirementRequest request,HttpContext http,AeroLinkDbContext db,IntegrationEventPublisher publisher,CancellationToken ct)
    {
        ...
        var existing=await db.IntegrationEvents.AsNoTracking().SingleOrDefaultAsync(x=>x.ProjectId==service.ProjectId&&x.IdempotencyKey==key,ct);if(existing is not null)return Results.Ok(new{scrId=existing.AggregateId,eventId=existing.Id,idempotentReplay=true,sourceHash});
        ...
        try
        {
            var now=DateTimeOffset.UtcNow;var number=await IdentifierAllocator.NextChangeRequestAsync(db,request.Type,ct);var scr=new SystemChangeRequest(number,0,service.ProjectId,request.TargetReleaseId,request.Title,$"OSLC RM requirement consumed from {sourceUri}.",request.Analysis,request.Solution,service.Name,now,request.Type);scr.AddRequirementChange(service.Name,request.LocalIdentifier,0,level,RequirementChangeKind.Introduce,request.Statement,request.Rationale,request.VerificationMethod,now);db.SystemChangeRequests.Add(scr);
            var receipt=await publisher.EnqueueAsync(service.ProjectId,"aerolink.oslc.requirement.consumed","SystemChangeRequest",scr.Id,new{scrId=scr.Id,sourceUri=sourceUri.ToString(),request.SourceEtag,request.ExternalIdentifier,request.LocalIdentifier,request.ConfigurationContext,mappingId=mapping.Id,mappingVersion=mapping.Version,mappingSha256=IntegrationSecurityService.Hash(mapping.MappingJson),sourceHash,consumedBy=service.Name,consumedAt=now,governance="Controlled draft change request; no approved requirement was directly mutated."},service.Name,now,ct,key);
            return Results.Accepted($"/api/scrs/{scr.Id}",new{scr.Id,scr.DisplayNumber,eventId=receipt.Id,idempotentReplay=false,sourceHash,mappingVersion=mapping.Version,governance="Controlled change proposal created; review, approval, and baseline selection remain required."});
        }
        catch(DomainException ex){return Results.BadRequest(new{error=ex.Message});}
    }
...
public sealed record OslcConsumeRequirementRequest(Guid TargetReleaseId,Guid MappingId,string SourceUri,string? SourceEtag,string? ExternalIdentifier,string LocalIdentifier,string Level,string Statement,string Rationale,string VerificationMethod,string? ConfigurationContext,string Title,string Analysis,string Solution,ChangeRequestType Type=ChangeRequestType.System);
