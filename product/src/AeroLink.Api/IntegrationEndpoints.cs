...
public static class IntegrationEndpoints
{
    ...
    private static async Task<IResult> OverviewAsync(Guid projectId, HttpContext http, AeroLinkDbContext db, CancellationToken ct)
    {
        ...
        var allDeliveries = await db.WebhookDeliveries.AsNoTracking().Where(x=>x.ProjectId==projectId).ToListAsync(ct);
        var now = DateTimeOffset.UtcNow;
        var deliveries = allDeliveries.Where(x=>x.State==WebhookDeliveryState.Delivering && x.ClaimExpiresAt is not null && x.ClaimExpiresAt<=now).OrderBy(x=>x.ClaimExpiresAt).ThenBy(x=>x.Id).Take(20)
            .Concat(allDeliveries.OrderByDescending(x=>x.CreatedAt).Take(80)).GroupBy(x=>x.Id).Select(x=>x.First()).ToList();
        ...
        return Results.Ok(new
        {
            ...
            metrics=new{activeIdentities=identities.Count(x=>x.State==ServiceIdentityState.Active),enabledWebhooks=subscriptions.Count(x=>x.IsEnabled),events24h=events.Count(x=>x.OccurredAt>=now.AddHours(-24)),deliverySuccess=allDeliveries.Count==0?100:(int)Math.Round(allDeliveries.Count(x=>x.State==WebhookDeliveryState.Delivered)*100d/allDeliveries.Count),deadLetters=allDeliveries.Count(x=>x.State==WebhookDeliveryState.DeadLettered),expiredClaims=allDeliveries.Count(x=>x.ClaimExpired(now)),connectors=connectors.Count,connectorsAttention=connectors.Count(x=>x.Status!="healthy"),mappingVersions=events.Count(x=>x.EventType==MappingRevision)},
            ...
            events=events.Take(40).Select(x=>new{x.Id,x.EventType,x.AggregateType,x.AggregateId,x.Actor,x.OccurredAt,state=x.State.ToString(),deliveryCount=allDeliveries.Count(d=>d.IntegrationEventId==x.Id)}),
            deliveries=deliveries.Select(x=>new{x.Id,x.IntegrationEventId,x.SubscriptionId,state=x.State.ToString(),x.AttemptCount,x.NextAttemptAt,x.ResponseStatusCode,x.LastError,x.CreatedAt,x.DeliveredAt,x.ClaimedBy,x.ClaimedAt,x.ClaimExpiresAt,needsAttention=x.ClaimExpired(now),attemptHistory=JsonSerializer.Deserialize<WebhookDeliveryAttempt[]>(x.AttemptHistoryJson)??[]}),
    ...
    {var item=await db.WebhookDeliveries.SingleOrDefaultAsync(x=>x.Id==id,ct);if(item is null)return Results.NotFound();if(!await http.HasProjectRoleAsync(db,identity,item.ProjectId,ct,ProgramRole.ConfigurationManager,ProgramRole.ProgramManager,ProgramRole.Administrator))return Results.Forbid();try{item.Replay(DateTimeOffset.UtcNow);await db.SaveChangesAsync(ct);return Results.Accepted();}catch(DomainException ex){return Results.Conflict(new{error=ex.Message,code="webhook_delivery_claim_active"});}catch(DbUpdateConcurrencyException){return Results.Conflict(new{error="The webhook delivery changed while it was being replayed.",code="webhook_delivery_changed"});}}
    ...
    private static async Task<IResult> ConsumeOslcRequirementAsync(OslcConsumeRequirementRequest request,HttpContext http,AeroLinkDbContext db,IntegrationEventPublisher publisher,IProjectLadderPolicyResolver policyResolver,CancellationToken ct)
    {
        ...
        var mapping=await db.RequirementImportMappings.AsNoTracking().SingleOrDefaultAsync(x=>x.Id==request.MappingId&&x.ProjectId==service.ProjectId,ct);if(mapping is null)return Results.BadRequest(new{error="The selected mapping does not belong to the service identity Project.",code="mapping_not_found"});var targetReleaseFailure=(await new ChangeRequestTargetReleaseGuard(db).ValidateAsync(service.ProjectId,request.TargetReleaseId,ct)).ToFailureResult();if(targetReleaseFailure is not null)return targetReleaseFailure;
        ...
        var policy=await policyResolver.ResolveAsync(service.ProjectId,ct);if(!Enum.TryParse<RequirementLevel>(request.Level,true,out var level))return Results.BadRequest(new{error="level must be System, HighLevel, or LowLevel.",code="invalid_level"});try{_ = policy.Definition(level);}catch(DomainException){return Results.BadRequest(new{error="level must be a configured ladder level.",code="invalid_level"});}
        try
        {
            var now=DateTimeOffset.UtcNow;var number=await IdentifierAllocator.NextChangeRequestAsync(db,request.Type,request.SoftwareLevel,ct,policy);var scr=new SystemChangeRequest(number,0,service.ProjectId,request.TargetReleaseId,request.Title,$"OSLC RM requirement consumed from {sourceUri}.",request.Analysis,request.Solution,service.Name,now,request.Type, ladderPolicy: policy, softwareLevel: request.SoftwareLevel);scr.AddRequirementChange(service.Name,request.LocalIdentifier,0,level,RequirementChangeKind.Introduce,request.Statement,request.Rationale,request.VerificationMethod,now,impactDispositionJson:RequirementAuthoringJson.PendingImpactDispositions, ladderPolicy: policy);db.SystemChangeRequests.Add(scr);
    ...
    {
        var service=http.ServiceIdentity();if(!service.HasScope("integrations:read"))return Results.Forbid();var subscriptions=await db.WebhookSubscriptions.AsNoTracking().CountAsync(x=>x.ProjectId==service.ProjectId&&x.IsEnabled,ct);var deliveries=await db.WebhookDeliveries.AsNoTracking().Where(x=>x.ProjectId==service.ProjectId).ToListAsync(ct);var events=db.Database.IsSqlite()
            ? await db.IntegrationEvents.FromSqlInterpolated($"SELECT * FROM integration_events WHERE ProjectId = {service.ProjectId} AND (EventType = {ConnectorConfigured} OR EventType = {ConnectorCheckpoint} OR EventType = {MappingRevision}) ORDER BY julianday(OccurredAt) DESC, Id LIMIT 1000").AsNoTracking().ToListAsync(ct)
            : await db.IntegrationEvents.AsNoTracking().Where(x=>x.ProjectId==service.ProjectId&&(x.EventType==ConnectorConfigured||x.EventType==ConnectorCheckpoint||x.EventType==MappingRevision)).OrderByDescending(x=>x.OccurredAt).Take(1000).ToListAsync(ct);var now=DateTimeOffset.UtcNow;var expiredClaims=deliveries.Count(x=>x.ClaimExpired(now));var connectors=BuildConnectorHealth(events,now);var status=deliveries.Any(x=>x.State==WebhookDeliveryState.DeadLettered)||expiredClaims>0||connectors.Any(x=>x.Status!="healthy")?"attention":"healthy";return Results.Ok(new{status,projectId=service.ProjectId,enabledSubscriptions=subscriptions,pendingDeliveries=deliveries.Count(x=>x.State is WebhookDeliveryState.Pending or WebhookDeliveryState.RetryScheduled),activeDeliveries=deliveries.Count(x=>x.State==WebhookDeliveryState.Delivering&&!x.ClaimExpired(now)),expiredClaims,deadLetters=deliveries.Count(x=>x.State==WebhookDeliveryState.DeadLettered),connectors,mappingRevisionEvents=events.Count(x=>x.EventType==MappingRevision),checkedAt=now});
    }
