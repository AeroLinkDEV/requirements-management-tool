...
public static class WorkspaceEndpoints
{
    ...
    var matchingProcedureTitleRevisionIds=await TestProcedureRevisionTitleProjection.MatchingRevisionIdsAsync(db,procedureSearchRevisionIds,q,ct);
            ...
            var executionSearchRevisionIds=await(from revision in db.TestProcedureRevisions.AsNoTracking() join procedure in db.TestProcedures.AsNoTracking().Where(x=>x.ProjectId==projectId) on revision.ProcedureId equals procedure.Id select revision.Id).ToListAsync(ct);
