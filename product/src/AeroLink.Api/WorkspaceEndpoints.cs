...
public static class WorkspaceEndpoints
{
    public static void MapWorkspaceEndpoints(this WebApplication app)
    {
        ...
        app.MapGet("/api/search",async(Guid projectId,Guid? releaseId,string query,int? limit,HttpContext http,AeroLinkDbContext db,CancellationToken ct)=>
        {
            ...
            var matchingProcedureTitleRevisionIds=await(from change in db.Set<TestProcedureChange>().AsNoTracking() join revision in db.TestProcedureRevisions.AsNoTracking() on new{ReviewId=(Guid?)change.TestChangeReviewId,change.Revision} equals new{ReviewId=revision.SourceTestChangeRequestId,revision.Revision} join procedure in db.TestProcedures.AsNoTracking().Where(x=>x.ProjectId==projectId) on revision.ProcedureId equals procedure.Id where procedure.BaseNumber==change.BaseNumber&&change.Title.ToLower().Contains(q)&&(effectiveProcedureRevisionIds==null?revision.Revision==db.TestProcedureRevisions.Where(other=>other.ProcedureId==procedure.Id).Max(other=>other.Revision):effectiveProcedureRevisionIds.Contains(revision.Id)) select revision.Id).Distinct().ToListAsync(ct);
            ...
            var matchingExecutionTitleRevisionIds=await(from change in db.Set<TestProcedureChange>().AsNoTracking() join revision in db.TestProcedureRevisions.AsNoTracking() on new{ReviewId=(Guid?)change.TestChangeReviewId,change.Revision} equals new{ReviewId=revision.SourceTestChangeRequestId,revision.Revision} join procedure in db.TestProcedures.AsNoTracking().Where(x=>x.ProjectId==projectId) on revision.ProcedureId equals procedure.Id where procedure.BaseNumber==change.BaseNumber&&change.Title.ToLower().Contains(q) select revision.Id).Distinct().ToListAsync(ct);
