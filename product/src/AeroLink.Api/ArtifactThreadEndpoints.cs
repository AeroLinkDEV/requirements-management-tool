...
/// <summary>
/// The artifact thread of #880 §5.3: one focal artifact's exact-revision chain across six lanes.
///
/// <para>
...
public static class ArtifactThreadEndpoints
{
    ...
    {
        app.MapGet("/api/artifact-thread", async (Guid projectId, string focalKind, Guid focalId,
            HttpContext http, AeroLinkDbContext db, CancellationToken ct) =>
        {
            ...
            var thread = await ArtifactThreadProjection.BuildAsync(db, projectId, kind, focalId, ct);
