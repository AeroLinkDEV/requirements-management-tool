...
public static class ProductLineEndpoints
{
    ...
    {
        app.MapProductLineCompletionEndpoints();
    ...
    private static async Task<IResult> OverviewAsync(Guid projectId,HttpContext http,AeroLinkDbContext db,CancellationToken ct)
    {
        ...
        return Results.Ok(new
        {
            ...
            variants=variants.Select(x=>new{x.Id,x.VariantKey,x.Name,state=x.State.ToString(),x.ApplicabilityJson,x.Version,componentCount=selections.Count(s=>s.VariantId==x.Id),baselineCount=baselines.Count(b=>b.VariantId==x.Id),latestBaseline=baselines.Where(b=>b.VariantId==x.Id).OrderByDescending(b=>b.Revision).Select(b=>new{b.Id,b.Revision,b.ManifestHash,b.CreatedAt}).FirstOrDefault()}),
    ...
    {var variant=await db.ProductVariants.AsNoTracking().SingleAsync(x=>x.Id==variantId,ct);var parts=await(from pick in db.VariantComponentSelections.AsNoTracking() where pick.VariantId==variantId join revision in db.ComponentStreamRevisions.AsNoTracking() on pick.ComponentRevisionId equals revision.Id join stream in db.ComponentStreams.AsNoTracking() on revision.StreamId equals stream.Id join component in db.ProductLineComponents.AsNoTracking() on stream.ComponentId equals component.Id orderby component.ComponentNumber,stream.StreamKey,revision.Revision select new{revisionId=revision.Id,component.ComponentNumber,component.Name,stream.StreamKey,revision.Revision,revision.ManifestHash,pick.ApplicabilityJson}).ToListAsync(ct);var libraries=await(from reuse in db.VariantLibraryReuses.AsNoTracking().Where(x=>x.VariantId==variantId) join library in db.ControlledLibraries.AsNoTracking() on reuse.LibraryId equals library.Id join selected in db.ControlledLibraryRevisions.AsNoTracking() on reuse.SelectedRevisionId equals selected.Id join upstream in db.ControlledLibraryRevisions.AsNoTracking() on reuse.LatestUpstreamRevisionId equals upstream.Id orderby library.LibraryNumber select new{reuseId=reuse.Id,library.LibraryNumber,library.Name,selectedRevisionId=selected.Id,selectedRevision=selected.Revision,selected.ManifestHash,latestUpstreamRevisionId=upstream.Id,latestUpstreamRevision=upstream.Revision,mode=reuse.Mode.ToString(),syncState=reuse.SynchronizationState.ToString(),reuse.ApplicabilityJson,reuse.DecisionRationale,reuse.DecidedBy,reuse.UpdatedAt}).ToListAsync(ct);var decisionRows=(await db.ComponentPropagationDecisions.AsNoTracking().Where(x=>x.VariantId==variantId).ToListAsync(ct)).OrderBy(x=>x.DecidedAt).ToList();var decisions=decisionRows.Select(x=>new{x.ComponentRevisionId,decision=x.Decision.ToString(),x.Rationale,x.DecidedBy,x.DecidedAt});var material=Canonicalize(JsonSerializer.Serialize(new{variant.VariantKey,variant.Name,variant.ApplicabilityJson,components=parts,libraries,propagationDecisions=decisions}));return(material,Hash(material));}
