...
app.Use(async (context, next) =>
{
    ...
    if(context.Request.Headers.TryGetValue("X-AeroLink-Build-Context",out var rawBuildContext))
    {
        ...
        var primaryMutationPrefixes=new[]{"/api/scr","/api/baseline","/api/build","/api/requirement","/api/enterprise-requirements","/api/test","/api/evidence","/api/release","/api/document","/api/trace","/api/problem-report","/api/downstream-assessment","/api/controlled-editing","/api/edit-sessions","/api/content/images","/api/reqif","/api/publication"};
