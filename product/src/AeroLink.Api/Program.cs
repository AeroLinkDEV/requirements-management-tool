...
app.MapGet("/api/scrs", async (IScrRepository repository, CancellationToken ct) =>
    Results.Ok((await repository.ListAsync(ct)).Select(ApiMap.ScrSummary)));
...
app.MapGet("/api/dashboard", async (AeroLinkDbContext db, CancellationToken ct) =>
{
    var scrs = await db.SystemChangeRequests.AsNoTracking().ToListAsync(ct);
