...
public static partial class ArtifactNumber
{
    ...
    [GeneratedRegex("^(?:SRCR|HLRCR|LLRCR|ICDCR)-(?:[0-9]{5}|[1-9][0-9]{5,})$")]
    ...
    ///
    /// A System change request is an SRCR, an Interface change request is an ICDCR, and software is an HLRCR
    /// or an LLRCR — never a single software prefix — because HLR and LLR change control are worked, reviewed
    /// and approved separately, and a reader who sees the identifier should already know which scope they are
    /// looking at.
    /// </summary>
    private static readonly string[] ChangeRequestPrefixes = ["SRCR-", "HLRCR-", "LLRCR-", "ICDCR-"];
    ...
    public static string ValidateBase(string value)
    {
        ...
        if (RetiredPrefixes.Any(prefix => normalized.StartsWith(prefix, StringComparison.Ordinal)))
            throw new DomainException("SCR and SWCR are retired. System change requests are SRCR, Interface change requests are ICDCR, and software change requests are HLRCR or LLRCR.");
