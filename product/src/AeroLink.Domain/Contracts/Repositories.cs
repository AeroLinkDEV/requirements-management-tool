...
public sealed record ScrQuery(Guid ProjectId, int Page = 1, int PageSize = 50, string? Search = null,
    ChangeRequestState? State = null, Guid? TargetReleaseId = null, string? BaseNumber = null, bool LatestRevisionOnly = false);
...
/// </param>
public sealed record ScrListItem(Guid Id, string BaseNumber, int Revision, string Title, ChangeRequestState State,
    ChangeRequestType Type, string AuthorId, Guid TargetReleaseId, int RequirementCount, DateTimeOffset UpdatedAt,
    ChangeRequestState? DeferredFromState = null, int RevisionCount = 1, string? RebaseRequiredReason = null);
