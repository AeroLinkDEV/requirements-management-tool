...
**715 test methods, 798 known invocations, 116 classes.** This source-exact forecast supports planning only;
...
**686 methods / 755 cases have direct host evidence**,
**22 methods / 36 cases are explicitly non-hosted**, and
**7 methods / 7 cases remain unknown** because their class contains a host fixture or factory but
the method body does not show the host operation.

| Intent | Tests | Cases | Classes | Correct level |
|---|---:|---:|---:|---|
| HTTP boundary: route, status, JSON shape | 426 | 480 | 95 | API (must stay hosted) |
| EF translation / relational constraints | 153 | 164 | 55 | Infrastructure (needs a database, not a host) |
| Authentication / authorization wiring | 83 | 87 | 48 | API (must stay hosted) |
| In-process logic with no HTTP and no client | 29 | 29 | 11 | Domain or Infrastructure (migration candidate) |
| Filesystem / evidence-root behaviour | 15 | 15 | 9 | API or Infrastructure (must stay hosted) |
...
The known hosted candidate share is **16 of 755 cases (2.1%)**,
