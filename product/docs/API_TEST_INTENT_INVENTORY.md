...
**991 test methods, 1131 known invocations, 157 classes.** This source-exact forecast supports planning only;
...
**934 methods / 1052 cases have direct host evidence**,
**38 methods / 52 cases are explicitly non-hosted**, and
**19 methods / 27 cases remain unknown** because their class contains a host fixture or factory but
the method body does not show the host operation.

| Intent | Tests | Cases | Classes | Correct level |
|---|---:|---:|---:|---|
| HTTP boundary: route, status, JSON shape | 611 | 697 | 123 | API (must stay hosted) |
| EF translation / relational constraints | 186 | 200 | 69 | Infrastructure (needs a database, not a host) |
| Authentication / authorization wiring | 111 | 129 | 65 | API (must stay hosted) |
| In-process logic with no HTTP and no client | 45 | 45 | 19 | Domain or Infrastructure (migration candidate) |
...
The known hosted candidate share is **18 of 1052 cases (1.7%)**,
