...
433 host constructions for roughly 442 hosted tests — very nearly one host per test. Disposal and
connection opening are negligible; the entire floor is building the host. The p95 at more than five
times the median means averages understate the tail.
...
current tree. The classifier is conservative about unknown method-level host use and custom
service/fault/interceptor/factory/template-copy configuration:

| Classification | Classes | Tests | Share |
|---|---:|---:|---:|
| reusable-host | 53 | 318 | 71.9% |
| converted (pilot) | 10 | 52 | 11.8% |
| fresh-host | 17 | 71 | 16.1% |
...
their isolation is explicitly reviewed. The generated artifact records the reason for every class; it is
...
reclassification above: its 62-class candidate pool is not identical to the current 53 reusable classes.
...
| Historical ideal-collapse builds removed | 314 of 372 (84%) |
...
shard and is not an upper bound on wall-clock improvement. The four reusable classes without telemetry,
the actual class-to-shard placement, xUnit parallelism, and synchronization costs remain unresolved.
