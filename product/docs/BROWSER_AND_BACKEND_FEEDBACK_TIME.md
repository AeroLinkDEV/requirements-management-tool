...
service/fault/interceptor/factory/template-copy configuration. A reviewed override file records the
...
|---|---:|---:|---:|
| reusable-host | 33 | 200 | 45.2% |
| converted (pilot) | 10 | 52 | 11.8% |
| fresh-host | 37 | 189 | 42.8% |
...
At the reviewed current tree, the 33 reusable classes contain 200 test methods. If every one were safely
converted to exactly one class-scoped host, the arithmetic upper bound would be **167 host constructions
removed (200 - 33)**. That is planning arithmetic, not measured wall-clock savings, and excludes the 20
reviewed holds.
