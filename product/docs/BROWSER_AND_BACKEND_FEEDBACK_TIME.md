...
Live activation and the first docs-only queue delivery complete the deferred strict-branch-protection item.
Ordinary behind branches no longer need a manual rebase; real conflicts and deliberately protected
CI-authority changes still require explicit disposition. The remaining multi-entry and deliberate-failure
qualification scenarios stay tracked on issue #549 until their evidence is recorded.
