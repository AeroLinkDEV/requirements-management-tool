...
if(Test-Path -LiteralPath $temporary){try{Remove-Item -LiteralPath (ConvertTo-AeroLinkArchiveIoPath $temporary) -Recurse -Force -ErrorAction Stop}catch{$cleanupError=$_;Write-Warning "Backup verification cleanup failed; the temporary directory remains at ${temporary}: $($cleanupError.Exception.Message)"}}
