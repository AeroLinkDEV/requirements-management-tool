...
$rootPrefix=[IO.Path]::GetFullPath($backupRoot)+[IO.Path]::DirectorySeparatorChar
if(-not ([IO.Path]::GetFullPath($archive).StartsWith($rootPrefix,[StringComparison]::OrdinalIgnoreCase))){throw 'Backup verification accepts archives only from product\.local\backups.'}
...
try{
 ...
 $manifest=@($manifestObject|ForEach-Object{$_});if($manifest.Count -eq 0){throw 'The backup manifest is empty.'}
 ...
 [pscustomobject]@{Valid=$true;Archive=$archive;ArchiveSha256=$actual;ManifestEntries=$manifest.Count;VerifiedAt=(Get-Date).ToUniversalTime().ToString('o')}
