...
try{
 ...
 $verified=& (Join-Path $PSScriptRoot 'Verify-AeroLinkBackup.ps1') -BackupArchive $archive -VerificationRoot $verificationRoot
 ...
 try{& (Join-Path $PSScriptRoot 'Verify-AeroLinkBackup.ps1') -BackupArchive $archive -VerificationRoot $verificationRoot|Out-Null;throw 'A corrupted relocated archive was accepted.'}catch{if($_.Exception.Message -notlike '*hash mismatch*'){throw}}
