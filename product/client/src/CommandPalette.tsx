...
const commandDefinitions:{label:string;view:View;discipline:Discipline;detail:string;icon:string;artifactKind?:string}[]=[
  ...
  {label:'System Test Change Requests',view:'testChangeRequests',discipline:'systemTest',detail:'The SYSTCRs controlling this build’s test procedures',icon:'◫'},
...
export default function CommandPalette({api,context,open,onClose,onNavigate}:Props){
  ...
  const artifactEntries=useMemo<PaletteEntry[]>(()=>results.filter(item=>!hiddenArtifactKinds.has(item.kind)).map(item=>({key:`artifact-${item.kind}-${item.id}`,category:'artifact',label:item.identifier,detail:item.title,state:item.state,view:item.kind==='change-request'?'scr':item.kind==='requirement'?'requirements':item.kind==='managed-document'?'managedDocuments':'artifact',discipline:(item.kind==='change-request'||item.kind==='requirement')&&item.discipline==='software'?'software':'system',artifactId:item.id,artifactKind:item.kind,icon:artifactAcronym(item.identifier,item.kind),updatedAt:item.updatedAt})),[results])
