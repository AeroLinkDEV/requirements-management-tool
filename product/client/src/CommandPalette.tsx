...
export default function CommandPalette({api,context,open,ladder,onClose,onNavigate}:Props){
  ...
  const openEntry=(entry:PaletteEntry)=>{remember({...entry,icon:canonicalIcon(entry)});const routeArtifactKind=entry.artifactKind==='change-request'&&entry.level==='Interface'?'Interface':entry.artifactKind;onNavigate(entry.view,entry.discipline,entry.artifactId,routeArtifactKind);onClose()}
  ...
  </div><aside className="palettePreview">{active?<><span className="previewIcon">{(()=>{const activeIcon=canonicalIcon(active);return isIconName(activeIcon)?<Icon name={activeIcon}/>:activeIcon})()}</span><p>{active.category==='artifact'?'CONTROLLED ARTIFACT':'WORKSPACE'}</p><h2>{active.label}</h2><div className="previewState">{active.state??'Ready to open'}</div><p className="previewDetail">{active.detail}</p><dl><div><dt>Scope</dt><dd>{active.discipline.replace('Test',' verification')}</dd></div><div><dt>Access</dt><dd>Current Program authority</dd></div>{active.updatedAt&&<div><dt>Updated</dt><dd><time dateTime={active.updatedAt}>{formatOrdinaryDateTime(active.updatedAt)}</time></dd></div>}</dl><footer><kbd>Enter</kbd><span>Open selection</span></footer></>:<><span className="previewIcon"><Icon name="search"/></span><h2>Search AeroLink</h2><p className="previewDetail">Jump directly to an authorized workspace or controlled artifact without losing your active release context.</p></>}</aside></div><footer><span><kbd>↑</kbd><kbd>↓</kbd> select <kbd>Enter</kbd> open</span><span>Results respect Program authority</span></footer></section></div>
}
