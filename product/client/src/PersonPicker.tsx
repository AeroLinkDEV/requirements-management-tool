...
export default function PersonPicker({api,projectId,value,name,index,label,excludeUserNames=[],allowedRoles,authority,onSelect}:{
 ...
 useEffect(()=>{const timer=setTimeout(async()=>{if(query.trim().length<1){setPeople([]);return}const scope=(authority?`&authority=${encodeURIComponent(authority)}`:"")+(allowedRoles?.length?`&roles=${encodeURIComponent(allowedRoles.join(","))}`:"");const response=await fetch(`${api}/api/directory?projectId=${projectId}&search=${encodeURIComponent(query)}&limit=10${scope}`);if(response.ok)setPeople(await response.json())},150);return()=>clearTimeout(timer)},[api,projectId,query,authority,allowedRoles?.join(",")])
