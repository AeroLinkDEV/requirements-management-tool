...
export default function PersonPicker({api,projectId,value,name,index,label,onSelect}:{
 api:string;projectId:string;value:string;name:string;index:number;label?:string;
 ...
 return <div className="personPicker"><input aria-label={label??`Approver ${index+1} search`} value={query} placeholder="Search name, title, or role…" autoComplete="off" onFocus={()=>setOpen(true)} onChange={event=>{setQuery(event.target.value);setOpen(true);setChosen(undefined);onSelect({userId:'',name:''})}}/>{value&&chosen&&describe(chosen)&&<small>{describe(chosen)}</small>}{open&&people.length>0&&<div className="personSuggestions">{people.map(person=><button type="button" key={person.id} data-user-name={person.userName} onClick={()=>{setChosen(person);onSelect({userId:person.userName,name:person.displayName});setQuery(person.displayName);setOpen(false)}}><i>{person.displayName.split(' ').map(x=>x[0]).join('').slice(0,2)}</i><span><b>{person.displayName}</b><small>{person.title}</small></span><em>{person.roles.join(' · ')}</em></button>)}</div>}</div>
}
