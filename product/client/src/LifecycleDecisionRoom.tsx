...
export default function LifecycleDecisionRoom({ api, projectId, activeReleaseId, releases, user, screen, selectedScrId, onBack, onOpenScr, onOpenImpact, onOpenDecision, onBackToReadiness, onOpenPlanning, onOpenVerification, onOpenDocuments, onOpenOperations }: {
  ...
  const openGate = (item: Gate) => {
    if (["coverage", "verification", "evidence"].includes(item.code)) onOpenVerification();
...
function ReadinessView({ detail, comparison, blockers, gate, onBack, onOpenDecision, onOpenImpact, openGate }: { detail: CampaignDetail; comparison?: Comparison; blockers: Gate[]; gate: (code: string) => Gate | undefined; onBack: () => void; onOpenDecision: () => void; onOpenImpact: (id: string) => void; openGate: (gate: Gate) => void }) {
  ...
  const gateStatus = (item?: Gate) => !item ? "Not evaluated" : item.complete ? "Complete" : item.total > 0 ? `${Math.max(0, item.total - item.completed)} gaps` : "Review required";
  const stages = [
  ...
  return <>
    ...
    <div className="readinessGrid">
      <section className="attentionCard"><div className="decisionCardTitle"><div><h2>What needs attention</h2><p>The exact work preventing a release decision</p></div><b>{blockers.length}</b></div>{blockers.slice(0, 3).map((item, index) => <article key={item.code}><span className="attentionIcon">!</span><div className="attentionCopy"><b>{item.name}</b><small>{item.detail}</small></div><div className="attentionOwner"><span className="attentionMeta">Owner</span><span className="ownerLine"><PersonAvatar userName={index === 0 ? "systems.lead" : index === 1 ? "test.engineer" : "release.manager"} displayName={owners[index]?.name} size="small" />{owners[index]?.name}</span></div><div className="attentionDue"><span className="attentionMeta">Due</span><b>{["24 Jul 2026", "26 Jul 2026", "30 Jul 2026"][index]}</b></div><em>{index === 0 ? "High" : "Medium"}</em><button onClick={() => openGate(item)}>Open exact work →</button></article>)}{!blockers.length && <div className="decisionEmpty"><b>✓ Every release gate is complete</b><span>The package is ready for formal authority.</span></div>}</section>
...
function HealthCard({ label, gate, tone }: { label: string; gate?: Gate; tone: string }) {
  ...
  return <article className={`${tone}${!measured && !gate?.complete ? " pendingMetric" : ""}`}><span>{label}</span><b>{measured || gate?.complete ? `${percent}%` : "Pending"}</b><small>{gate?.complete ? "Target achieved" : measured ? `${Math.max(0, gate!.total - gate!.completed)} checks remaining` : "Evaluation not started"}</small><i><em style={{ width: `${percent}%` }} /></i></article>;
}
...
function ImpactView({ detail, scr, affected, selectedIndex, selected, onSelect, onBack, onOpenScr, onOpenVerification }: { detail: CampaignDetail; scr?: ScrDetail; affected: AffectedRequirement[]; selectedIndex: number; selected?: AffectedRequirement; onSelect: (index: number) => void; onBack: () => void; onOpenScr: () => void; onOpenVerification: () => void }) {
  ...
  return <>
    ...
    <div className="impactGrid">
      <section className="affectedCard"><div className="decisionCardTitle"><div><h2>Affected requirements</h2><p>{countLabel(affected.length, "controlled revision")}</p></div></div><div className="affectedSearch">⌕ Search requirements</div>{affected.map((item, index) => <button className={index === selectedIndex ? "selected" : ""} key={item.id} onClick={() => onSelect(index)}><i className={item.kind.toLowerCase()}>{item.kind === "Introduce" ? "+" : item.kind === "Retire" ? "−" : "○"}</i><span><b>{compactNumber(item.displayNumber)}</b><small>{item.statement.slice(0, 58)}{item.statement.length > 58 ? "…" : ""}</small></span><em>{item.kind}</em></button>)}</section>
...
function DecisionView({ detail, comparison, documents, blockers, decision, onDecision, onBack, onRecord, onOpenDocuments, openGate }: { detail: CampaignDetail; comparison?: Comparison; documents: DocumentRecord[]; blockers: Gate[]; decision: "Approve" | "Approve with conditions" | "Return for rework"; onDecision: (decision: "Approve" | "Approve with conditions" | "Return for rework") => void; onBack: () => void; onRecord: () => void; onOpenDocuments: () => void; openGate: (gate: Gate) => void }) {
  ...
  return <>
    <PageHeader eyebrow={`FMS LIVE / RELEASE ${detail.release} / DECISION PACKAGE`} title="Release Evidence & Decision" subtitle={`Review the exact evidence supporting candidate baseline ${compactNumber(detail.baseline)}.`} onBack={onBack} backLabel="Back to readiness" actionLabel="Record decision" onAction={onRecord} />
    ...
    <div className="evidenceGrid"><section className="evidenceChecklist"><div className="decisionCardTitle"><div><h2>Evidence checklist</h2><p>Every claim opens its authoritative record</p></div></div><div>{checklist.map(([label, status, code]) => { const item = detail.readiness.gates.find((candidate) => candidate.code === code); const complete = item?.complete ?? status.includes("Complete"); return <button key={label} onClick={() => item ? openGate(item) : onOpenDocuments()}><i className={complete ? "complete" : "attention"}>{complete ? "✓" : "△"}</i><span><b>{label}</b><small>{status}</small></span><em>View →</em></button>; })}</div></section>
      ...
      <section className="approvalPath"><div className="decisionCardTitle"><div><h2>Approval path</h2><p>Ordered electronic authorization</p></div></div><div className="approvalTimeline">{approvals.map((item) => <article key={item.position} className={item.state.toLowerCase()}><i>{item.position}</i><PersonAvatar userName={item.approverId} displayName={item.person?.name ?? item.approverName} size="small" /><span><b>{item.person?.role ?? item.approverName}</b><small>{item.person?.name ?? item.approverName} · {item.state}</small></span></article>)}</div><div className="approvalProgress"><span>{approved} of {Math.max(detail.approvals.length, approvals.length)} approvals complete</span><i><em style={{ width: `${approved / Math.max(1, detail.approvals.length || approvals.length) * 100}%` }} /></i></div></section>
      <aside className="decisionSide"><section className="decisionBlockers"><h2>△ Decision blockers</h2>{blockers.slice(0, 3).map((item) => <button key={item.code} onClick={() => openGate(item)}><span>{Math.max(0, item.total - item.completed)} · {item.name}</span><b>Open →</b></button>)}</section><section className="yourDecision"><h2>Your decision</h2><div>{(["Approve", "Approve with conditions", "Return for rework"] as const).map((item) => <button className={decision === item ? "selected" : ""} onClick={() => onDecision(item)} key={item}>{item}</button>)}</div><p>A rationale and electronic signature are required.</p><button className="decisionPrimary" onClick={onRecord}>Record decision</button></section></aside>
    </div>
