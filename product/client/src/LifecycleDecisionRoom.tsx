...
type RequirementImpact = {
  ...
  tests: { id: string; revisionId: string; displayNumber: string; title: string; level?: string; state: string; isSuspect: boolean; coverageState: "Confirmed" | "Suspect" }[];
...
export default function LifecycleDecisionRoom({ api, projectId, activeReleaseId, releases, user, screen, selectedScrId, onBack, onOpenScr, onOpenImpact, onOpenDecision, onBackToReadiness, onOpenVerification, onOpenDocuments, onOpenOperations }: {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // The predecessor is the nearest earlier version, whatever order the caller supplied (#1091 RR-1).
      const ordered = [...releases].sort((a, b) => parseFloat(a.version) - parseFloat(b.version));
      const activeIndex = ordered.findIndex((item) => item.id === activeReleaseId);
      if (activeIndex > 0) {
        const compareResponse = await fetch(`${api}/api/release-comparison?projectId=${projectId}&fromReleaseId=${ordered[activeIndex - 1].id}&toReleaseId=${activeReleaseId}`);
  ...
  const sign = async (password: string, meaning: string) => {
    if (!detail) return;
    const response = await fetch(`${api}/api/release-campaigns/${detail.id}/approve`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ password, meaning, expectedManifestHash: detail.releaseHash ?? "" }) });
...
function ReadinessView({ detail, comparison, blockers, gate, onBack, onOpenDecision, onExploreChanges, openGate }: { detail: CampaignDetail; comparison?: Comparison; blockers: Gate[]; gate: (code: string) => Gate | undefined; onBack: () => void; onOpenDecision: () => void; onExploreChanges: () => void; openGate: (gate: Gate) => void }) {
  ...
  return <>
    <PageHeader eyebrow={`BUILD ${detail.release}`} title="Release Readiness" subtitle="A decision-ready view of the complete lifecycle." onBack={onBack} backLabel="Command Center" secondaryLabel={`Explore changes vs ${comparison?.fromRelease ?? "Predecessor"} →`} onSecondary={onExploreChanges} actionLabel="Review release evidence" onAction={onOpenDecision} />
    ...
    <div className="readinessGrid">
      ...
      <aside className="comparisonCard"><div className="decisionCardTitle"><div><h2>Release comparison</h2><p>Released baseline to current candidate</p></div></div><div className="versionFlow"><div><b>{comparison?.fromRelease ?? "Predecessor"}</b><span>Released</span></div><i>→</i><div><b>{detail.release}</b><span>Candidate</span></div></div><dl>{[["Introduced", comparison?.summary.added ?? 0, "added"],["Modified", comparison?.summary.modified ?? comparison?.summary.proposed ?? 0, "modified"],["Retired", comparison?.summary.retired ?? 0, "retired"],["Unchanged", comparison?.summary.unchanged ?? 0, "unchanged"]].map(([label, value, tone]) => <div key={String(label)} className={String(tone)}><dt>{label}</dt><dd>{comparison?.toMaterialized ? value : "—"}</dd></div>)}</dl>{comparison && !comparison.toMaterialized && <p className="comparisonPending">Counts become exact when the candidate baseline is materialized.</p>}<button onClick={onExploreChanges}>Explore baseline changes →</button></aside>
    </div>
...
function ChangesView({ detail, comparison, onBack, onOpenImpact, onOpenDecision }: { detail: CampaignDetail; comparison?: Comparison; onBack: () => void; onOpenImpact: (id: string) => void; onOpenDecision: () => void }) {
  ...
  return <>
    <PageHeader eyebrow={`BUILD ${detail.release}`} title="Baseline changes" subtitle={`Every controlled change between ${comparison?.fromRelease ?? "the released build"} and ${detail.release}.`} onBack={onBack} backLabel="Back to readiness" actionLabel="Review release evidence" onAction={onOpenDecision} />
...
function ImpactView({ detail, scr, affected, selectedIndex, selected, onSelect, onBack, onOpenScr, onOpenVerification }: { detail: CampaignDetail; scr?: ChangeRequestDetail; affected: AffectedRequirement[]; selectedIndex: number; selected?: AffectedRequirement; onSelect: (index: number) => void; onBack: () => void; onOpenScr: () => void; onOpenVerification: () => void }) {
  ...
  return <>
    <PageHeader eyebrow={`BUILD ${detail.release} / ${compactNumber(scr.displayNumber)}`} title="Change Impact Review" subtitle="Understand the full lifecycle effect before making a decision." onBack={onBack} backLabel="Back to readiness" actionLabel="Open in Changes" onAction={onOpenScr} />
