...
function ReadinessView({ detail, comparison, blockers, gate, onBack, onOpenDecision, onOpenImpact, openGate }: { detail: CampaignDetail; comparison?: Comparison; blockers: Gate[]; gate: (code: string) => Gate | undefined; onBack: () => void; onOpenDecision: () => void; onOpenImpact: (id: string) => void; openGate: (gate: Gate) => void }) {
  ...
  return <>
    ...
    <div className="readinessGrid">
      ...
      <section className="attentionCard"><div className="decisionCardTitle"><div><h2>What needs attention</h2><p>The exact work preventing a release decision</p></div><b>{blockers.length}</b></div>{blockers.map((item) => <article key={item.code}><span className="attentionIcon">!</span><div className="attentionCopy"><b>{item.name}</b><small>{item.detail}</small></div><div className="attentionRemaining"><span className="attentionMeta">Remaining</span><b>{item.total > 0 ? `${Math.max(0, item.total - item.completed)} of ${item.total}` : "Not started"}</b></div><div className="attentionAction"><span className="attentionMeta">To clear this</span><small>{item.action}</small></div><button onClick={() => openGate(item)}>Open exact work →</button></article>)}{!blockers.length && <div className="decisionEmpty"><b>✓ Every release gate is complete</b><span>The package is ready for formal authority.</span></div>}</section>
