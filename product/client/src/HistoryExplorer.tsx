...
export default function HistoryExplorer({api,projectId,releases,activeReleaseId,scope,initialStateIntent,onStateIntentChange,onTypeIntentChange,onBack,onOpenScr}:Props){
 ...
 const load=useCallback(async()=>{const params=new URLSearchParams({projectId,page:String(scrPage),pageSize:'50',releaseId:activeReleaseId});if(scope!=='All')params.set('type',scope);if(stateIntent)params.set('state',stateIntent);if(query)params.set('search',query)
  const [a,c]=await Promise.all([fetch(`${api}/api/history/scrs?${params}`),fetch(`${api}/api/builds?projectId=${projectId}&releaseId=${activeReleaseId}&search=${encodeURIComponent(query)}`)])
  if(a.ok){const body=await a.json();setScrs(body.items);setScrTotal(body.totalCount);setScrTotalPages(Math.max(1,body.totalPages))}if(c.ok)setBuilds(await c.json())},[activeReleaseId,api,projectId,query,scope,scrPage,stateIntent])
 useEffect(()=>{const timer=setTimeout(load,180);return()=>clearTimeout(timer)},[load])
 useEffect(()=>{setScrPage(1)},[activeReleaseId])
 ...
 const prepareCreate=async()=>{if(activeRelease?.isReleased)return;const list=(await fetch(`${api}/api/baselines?projectId=${projectId}&releaseId=${activeReleaseId}`).then(x=>x.json())).map((x:Baseline)=>({...x,releaseId:activeReleaseId}));setBaselines(list.filter((x:Baseline)=>x.state==='Frozen'));setShowCreate(true)}
 const createBuild=async(e:FormEvent<HTMLFormElement>)=>{e.preventDefault();const form=new FormData(e.currentTarget),baseline=baselines.find(x=>x.id===form.get('baselineId'));if(!baseline)return;const response=await fetch(`${api}/api/builds`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({projectId,releaseId:baseline.releaseId,baselineId:baseline.id,buildNumber:form.get('buildNumber'),description:form.get('description')})});if(!response.ok){setError((await response.json()).error||'Build could not be recorded.');return}setShowCreate(false);await load();setTab('builds')}
