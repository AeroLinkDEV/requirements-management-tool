...
const conclusionText=(row:Assessment)=>{
  ...
  if(row.outcome==='ChangeRequestsLinked')return `A ${level} requirement change is required, and is controlled by the linked Draft SWCR.`
  if(row.outcome==='ChangeRequired')return `A ${level} requirement change is required. No Draft SWCR carries it yet.`
  return ''
}
const outcomeWords=(outcome:string)=>outcome==='NoChangeRequired'?'no change required'
  :outcome==='ChangeRequired'?'change required'
    :outcome==='ChangeRequestsLinked'?'change required, SWCR linked':'undecided'
...
const engineeringStatus=(row:Assessment)=>{
  ...
  if(row.outcome==='ChangeRequired')return `Draft SWCR needed`
