    ...
    {rows.map((row,index)=><article className={`downstreamAssessment ${row.state.toLowerCase()}`} key={row.id}>
      ...
      <div className="downstreamActions">
        ...
        {row.capabilities.canEdit&&<>
          <button type="button" className="quiet" disabled={busy===row.id} onClick={()=>{const rationale=prompt('Why is no downstream requirement change required?');if(rationale?.trim())void act(row.id,'no-change',{rationale})}}>No change required</button>
        ...
        {row.capabilities.canApprove&&<button type="button" onClick={()=>void act(row.id,'approve')}>Approve</button>}{row.capabilities.canReturn&&<button type="button" className="quiet" onClick={()=>{const rationale=prompt('What must be updated?');if(rationale?.trim())void act(row.id,'return',{rationale})}}>Return</button>}
      </div>
    </article>)}
