    ...
    <aside className="appNavigation">
      ...
      <nav className="primaryNavigation" aria-label="Primary navigation">
        ...
        <details className="navGroup" open={view==="verification"||view==="testingCoverage"||view==="procedureExplorer"||view==="testResults"||(view==="documents"&&(discipline==="systemTest"||discipline==="softwareTest"))}><summary>VERIFICATION</summary><div className="navScopeSwitch" role="group" aria-label="Verification scope"><button type="button" aria-pressed={verificationScope==="systemTest"} onClick={()=>onNavigate("verification","systemTest")}>System</button><button type="button" aria-pressed={verificationScope==="softwareTest"} onClick={()=>onNavigate("verification","softwareTest")}>Software</button></div>{verificationScope==="softwareTest"
          ? <>{item("HLR Change Requests","testingCoverage","◫","softwareTest","Software HLR Test Change Requests","HighLevel")}{item("HLR Test Procedure Explorer","procedureExplorer","≡","softwareTest","Software HLR Test Procedure Explorer","HighLevel")}{item("HLR Test Results","testResults","▦","softwareTest","Software HLR Test Results","HighLevel")}{item("LLR Change Requests","testingCoverage","◫","softwareTest","Software LLR Test Change Requests","LowLevel")}{item("LLR Test Procedure Explorer","procedureExplorer","≡","softwareTest","Software LLR Test Procedure Explorer","LowLevel")}{item("LLR Test Results","testResults","▦","softwareTest","Software LLR Test Results","LowLevel")}</>
          : <>{item("Change Requests","testingCoverage","◫","systemTest","System Test Change Requests")}{item("Test Procedure Explorer","procedureExplorer","≡","systemTest","System Test Procedure Explorer")}{item("Test Results","testResults","▦","systemTest","System Test Results")}</>}{item("Generated Documents","documents","▤",verificationScope,verificationScope==="softwareTest"?"Generated Software Verification Documents":"Generated System Verification Documents")}</details>
...
function App() {
  ...
  const approvalConfigurationPath=projectAreaPath(openProjectSlug,"approvalConfiguration");
  ...
  const showApprovalConfiguration=()=>{setView("approvalConfiguration");history.pushState({},"",approvalConfigurationPath)};
  if(view==="builds")return <SoftwareBuildsLanding user={user} releases={project?.releases??[]} onProjectOverview={showProjects} onImportedBaselines={showImports} onPersonnel={showPersonnel} onApprovalConfiguration={showApprovalConfiguration} onOpenBuild={(selected)=>{if(!active||!project||!project.releases.some(item=>item.id===selected.id))return;setSelectedReleaseId(selected.id);setView("dashboard");history.pushState({},"",routePath({programId:active.program.id,projectId:project.project.id,releaseId:selected.id},"dashboard"))}} onSignOut={signOut}/>;
