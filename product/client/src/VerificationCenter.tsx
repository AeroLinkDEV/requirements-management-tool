...
/// Answers whether the call asking is still the most recent one, so a reader's screen is never written by a
/// reply they have already moved on from.
type Current=()=>boolean

/// Only the newest call of a loader may apply what it read.
///
/// Every loader below is rebuilt when a filter changes, so changing a filter starts a second request while the
/// first is still in flight, and nothing ordered the replies. The broad query is the slow one, so the narrow
/// filtered reply routinely arrived first and was then overwritten by the unfiltered reply behind it: the
/// reader typed a search, saw the one procedure they wanted, and watched the full list they had just filtered
/// away come back over the top of it, with their search term still sitting in the box. Recovering needed
/// another keystroke, and on a slow machine that one raced too.
///
/// A ticket taken per call and rechecked after the awaits is enough. The stale reply is still parsed — it is
/// already on the wire and cancelling it buys nothing here — it simply does not get to speak.
function useLatestOnly(read:(isCurrent:Current)=>Promise<void>){
 const ticket=useRef(0)
 return useCallback(()=>{const mine=++ticket.current;return read(()=>mine===ticket.current)},[read])
}
...
export default function VerificationCenter({api,programId,projectId,releaseId,buildName,readOnly,scope,user,correctiveProblemReportId,onBack}:Props){
 ...
 const [testChangeReviews,setTestChangeReviews]=useState<TestChangeReview[]>([])
 ...
 const canTest=!readOnly&&(user.isAdministrator||!!selectedProgram?.roles.includes('TestEngineer'))
 const canApprove=!readOnly&&(user.isAdministrator||!!selectedProgram?.roles.includes('Approver'))
 const canLead=!readOnly&&(user.isAdministrator||!!selectedProgram?.roles.includes('TestLead'))
 const canDecideImpact=canTest||canLead
 const hasMaterializedBaseline=baselines.length>0
 const readWorkspace=useCallback(async(isCurrent:Current)=>{const [a,b,c,d,e]=await Promise.all([fetch(`${api}/api/baselines?projectId=${projectId}&releaseId=${releaseId}`),fetch(`${api}/api/builds?projectId=${projectId}&releaseId=${releaseId}`),fetch(`${api}/api/test-procedures?projectId=${projectId}&releaseId=${releaseId}&scope=${scope}&search=${encodeURIComponent(procedureQuery)}&state=${procedureState}&outcome=${procedureOutcome}&page=${procedurePage}&pageSize=25`),fetch(`${api}/api/test-executions?projectId=${projectId}&releaseId=${releaseId}${buildId?`&buildId=${buildId}`:''}`),fetch(`${api}/api/build-context?projectId=${projectId}&releaseId=${releaseId}`)]);const bs:Baseline[]=a.ok?await a.json():[];const context=e.ok?await e.json():undefined;const releaseBuilds:Build[]|undefined=b.ok?await b.json():undefined;const paged=c.ok?await c.json():undefined;const runs=d.ok?await d.json():undefined;const inherited=context?.effectiveBaseline&&context.inheritedBaseline?{id:context.effectiveBaseline.id,displayNumber:`${context.effectiveBaseline.baseNumber}.${String(context.effectiveBaseline.revision).padStart(2,'0')}`,name:`${context.effectiveBaseline.name} · inherited from ${context.effectiveBaseline.displayNumber}`,state:'Frozen',requirementsMaterializedAt:context.effectiveBaseline.requirementsMaterializedAt}:undefined;const available=[...bs.filter(x=>x.requirementsMaterializedAt),...(inherited&&!bs.some(x=>x.id===inherited.id)?[inherited]:[])];if(!isCurrent())return;setBaselines(available);if(releaseBuilds){setBuilds(releaseBuilds);setBuildId(current=>releaseBuilds.some(x=>x.id===current)?current:releaseBuilds[0]?.id||'')}if(paged){setProcedures(paged.items);setProcedureTotal(paged.totalCount);setProcedurePages(paged.totalPages)}if(runs)setExecutions(runs);setBaselineId(current=>available.some(x=>x.id===current)?current:context?.effectiveBaselineId||available[0]?.id||'')},[api,projectId,releaseId,buildId,scope,procedureQuery,procedureState,procedureOutcome,procedurePage])
 const readCoverage=useCallback(async(isCurrent:Current)=>{if(!baselineId&&!buildId)return;const key=buildId?`buildId=${buildId}`:`baselineId=${baselineId}`;const [a,b]=await Promise.all([fetch(`${api}/api/verification-coverage?projectId=${projectId}&${key}`),fetch(`${api}/api/requirements?projectId=${projectId}&baselineId=${baselineId||builds.find(x=>x.id===buildId)?.baselineId}&scope=${scope}&includeRetired=false&page=1&pageSize=200`)]);const raw:Coverage|undefined=a.ok?await a.json():undefined;const requirementPage=b.ok?await b.json():undefined;if(!isCurrent())return;if(raw){const items=raw.items.filter(x=>scope==='System'?x.displayNumber.startsWith('SYSR-'):!x.displayNumber.startsWith('SYSR-'));setCoverage({...raw,items,total:items.length,covered:items.filter(x=>x.covered).length,verified:items.filter(x=>x.verified).length,uncovered:items.filter(x=>!x.covered).length})}if(requirementPage)setRequirements(requirementPage.items)},[api,projectId,baselineId,buildId,builds,scope])
 const readImpact=useCallback(async(isCurrent:Current)=>{const response=await fetch(`${api}/api/releases/${releaseId}/verification-impact`);const body=response.ok?await response.json():undefined;if(!isCurrent())return;if(body)setImpact(body);else{recordClientOperationFailure('verification.impact.load',new Error(`HTTP ${response.status}`));setError('The verification impact for this build could not be loaded.')}},[api,releaseId])
 // A failed load used to be silent, so a 500 from this endpoint rendered as "No test change reviews" — a
 // broken workspace that looked merely empty. An empty queue and an unavailable queue are not the same answer.
 const readReviews=useCallback(async(isCurrent:Current)=>{const response=await fetch(`${api}/api/releases/${releaseId}/test-change-reviews`);const body=response.ok?await response.json():undefined;if(!isCurrent())return;if(body)setTestChangeReviews(body);else{recordClientOperationFailure('verification.reviews.load',new Error(`HTTP ${response.status}`));setError('The test change reviews for this build could not be loaded.')}},[api,releaseId])
 const load=useLatestOnly(readWorkspace),loadCoverage=useLatestOnly(readCoverage),loadImpact=useLatestOnly(readImpact),loadReviews=useLatestOnly(readReviews)
 useEffect(()=>{load()},[load]);useEffect(()=>{loadCoverage()},[loadCoverage]);useEffect(()=>{loadImpact()},[loadImpact]);useEffect(()=>{loadReviews()},[loadReviews])
 ...
 const resolveImpact=async(e:FormEvent<HTMLFormElement>)=>{e.preventDefault();if(!resolving||mutationBusy)return;setMutationBusy(true);setError('');const form=new FormData(e.currentTarget);const outcomeValue=String(form.get('outcome'));const procedureId=String(form.get('procedureId')||'');try{await apiRequest(`${api}/api/verification-impact/${resolving.id}/resolve`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({outcome:outcomeValue,rationale:form.get('rationale'),procedureId:outcomeValue==='ProcedureCoverageConfirmed'?procedureId:null,procedureChangeAction:outcomeValue==='NoTestRequired'?'NoTestRequired':form.get('procedureChangeAction'),preReleaseEvidenceRequired:outcomeValue==='ProcedureCoverageConfirmed'&&form.get('preReleaseEvidenceRequired')==='on',retargetedRequirementRevisionId:outcomeValue==='ProcedureRetargeted'?String(form.get('retargetedRequirementRevisionId')||'')||null:null})});setResolving(undefined);await loadImpact();await loadReviews();await loadCoverage()}catch(error){recordClientOperationFailure('verification.impact.resolve',error);setError(operationError(error,'The decision could not be recorded.'))}finally{setMutationBusy(false)}}
 const reviewAction=async(review:TestChangeReview,action:'submit'|'approve')=>{if(mutationBusy)return;setMutationBusy(true);setError('');try{await apiRequest(`${api}/api/test-change-reviews/${review.id}/${action}`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(action==='approve'?{rationale:'Test procedure changes reviewed and approved for this software build.'}:{})});await loadReviews()}catch(error){recordClientOperationFailure(`verification.review.${action}`,error);setError(operationError(error,`The test change review could not be ${action==='submit'?'submitted':'approved'}.`))}finally{setMutationBusy(false)}}
 ...
 const scopedReviews=testChangeReviews.filter(review=>scope==='System'
 ...
 return <main className="verificationPage"><header><div><button className="back" onClick={onBack}>← Command Center</button><p className="eyebrow">VERIFICATION CONTROL / EXTERNAL EXECUTION</p><h1>{scope==='System'?'System':'Software'} Verification &amp; Evidence</h1><p>Control procedures, coverage, human result determinations, evidence, and retest history.</p></div><button disabled={!canTest||!hasMaterializedBaseline} title={!canTest?'Test Engineer authority is required in this Program.':!hasMaterializedBaseline?'Materialize the software build requirements before creating a procedure.':undefined} onClick={()=>setCreating(true)}>+ New Test Procedure</button></header>{error&&<div className="workspaceError" role="alert" aria-live="assertive">{error}</div>}
 <section className="verificationContext" aria-label="Active software build"><div className="verificationContextIdentity"><small>ACTIVE SOFTWARE BUILD</small><b>{buildName}</b><span>Baseline and software build are one controlled configuration. Exit the workspace to select another build.</span></div></section>
 {!hasMaterializedBaseline&&<section className="materializationPrerequisite" role="status"><div><b>Procedure authoring waits for governed requirement materialization</b><p>This build has no immutable requirement revisions yet, so a new procedure cannot be bound to an exact target. Existing inherited procedures remain visible against their predecessor revisions; planned work for new or modified requirements remains in Procedure alignment and cannot count as confirmed coverage yet.</p></div><div><span>Current limitation</span><b>Requirement materialization is not exposed in this simplified workspace.</b></div></section>}
 ...
 <section className="verificationStreams">
  <div><small>PRE-RELEASE PROCEDURE ALIGNMENT</small><b>Keep test procedures aligned with approved requirement changes.</b></div>
  <div><small>EVIDENCE &amp; RESULTS</small><b>Capture execution evidence before release only when the test change review requires it; otherwise continue after release.</b></div>
 </section>
 <nav className="verificationTabs" aria-label="Verification workspace"><button className={workspaceTab==='impact'?'active':''} onClick={()=>setWorkspaceTab('impact')}>Procedure alignment <span className={outstandingImpact.length?'attention':''}>{outstandingImpact.length}</span></button><button className={workspaceTab==='coverage'?'active':''} onClick={()=>setWorkspaceTab('coverage')}>Requirement coverage <span>{coverage?.total??0}</span></button><button className={workspaceTab==='procedures'?'active':''} onClick={()=>setWorkspaceTab('procedures')}>Test procedures <span>{procedureTotal}</span></button><button className={workspaceTab==='executions'?'active':''} onClick={()=>setWorkspaceTab('executions')}>Evidence &amp; results <span>{executions.length}</span></button></nav>
 <div className="verificationFocus">
  {workspaceTab==='impact'&&<section className="verificationCard">
   <div className="cardTitle"><h2>Test procedure alignment</h2><p>Each approved change request creates controlled System, HLR, or LLR test change review work for this software build.</p></div>
   <section className="testChangeReviewList" aria-label="Test change reviews">
    {scopedReviews.map(review=><article className={`testChangeReview ${review.state.toLowerCase()}`} key={review.id}>
      {/* Its own controlled number, and the changes it answers for. A package usually covers the one change
          request it was raised from; when a verification engineer folds a second in because the two are
          sensibly tested together, that has to be readable without opening it. */}
      <div><b>{review.displayNumber??review.sourceChangeRequestNumber}</b><i>{review.discipline==='HighLevelSoftware'?'Software HLR':review.discipline==='LowLevelSoftware'?'Software LLR':'System'} test change request</i><span>{review.state}</span></div>
      <small className="coveredChanges">Covers {(review.coveredChangeRequests??[{id:review.id,number:review.sourceChangeRequestNumber,originating:true}]).map(x=>x.number).join(', ')}</small>
      <p>{review.resolvedItems} of {review.totalItems} procedure decisions complete{review.preReleaseEvidenceItems?` · ${review.preReleaseEvidenceItems} require pre-release evidence`:''}.</p>
      {review.state==='Open'&&review.resolvedItems===review.totalItems&&canDecideImpact&&<button onClick={()=>reviewAction(review,'submit')}>Submit procedure review</button>}
      {review.state==='InReview'&&canApprove&&<button onClick={()=>reviewAction(review,'approve')}>Approve procedure review</button>}
      {review.state==='Approved'&&<small>Approved by <PersonName userName={review.approvedBy}/>{review.approvedAt&&<> · {new Date(review.approvedAt).toLocaleDateString()}</>}</small>}
    </article>)}
    {!scopedReviews.length&&<div className="verificationEmpty"><b>No test change reviews</b><span>Approving a change request creates the appropriate discipline review automatically.</span></div>}
   </section>
   <div className="impactSummary">
    ...
    <article><b>{scopedImpact.length-outstandingImpact.length}</b><span>Decided</span></article>
...
{assigning&&<div className="impactDialog" role="dialog" aria-label="Assign verification work"><form onSubmit={e=>{e.preventDefault();assignImpact(assigning,String(new FormData(e.currentTarget).get('engineerId')||''))}}><h2>Assign {assigning.subjectDisplayNumber}</h2><p>Distribute this decision to a verification engineer.</p><label>Verification engineer<input name="engineerId" required placeholder="username"/></label><div><button type="button" className="outline" onClick={()=>setAssigning(undefined)}>Cancel</button><button>Assign</button></div></form></div>}
 {resolving&&<div className="impactDialog" role="dialog" aria-label="Record verification decision"><form onSubmit={resolveImpact}><h2>Decide {resolving.subjectDisplayNumber}</h2><p>{impactLabels[resolving.trigger]??resolving.trigger}{resolving.declaredVerificationMethod?` · author declared ${resolving.declaredVerificationMethod}`:''}</p>
  <label>Decision<select name="outcome" defaultValue={resolving.trigger==='ProcedureOrphaned'?'ProcedureRetired':'ProcedureCoverageConfirmed'}>{resolving.trigger==='ProcedureOrphaned'?<><option value="ProcedureRetired">Procedure retired</option><option value="ProcedureRetargeted">Procedure moved to another requirement</option><option value="ProcedureRetained">Procedure deliberately retained</option></>:<><option value="ProcedureCoverageConfirmed">An approved procedure covers this</option><option value="NoTestRequired">No test required</option></>}</select></label>
      {/* Offered whenever a procedure has been stranded, and sent only with the decision that moves it. The
          decision control above is uncontrolled, so showing this conditionally would mean threading form
          state through for one field; saying plainly what it is for costs the reader less than that costs
          everyone reading this component. */}
      {resolving.trigger==='ProcedureOrphaned'&&<label>Requirement it moves to <small>Used only by "Procedure moved to another requirement".</small><select name="retargetedRequirementRevisionId" defaultValue=""><option value="">Choose a requirement…</option>{requirements.map(x=><option key={x.revisionId} value={x.revisionId}>{x.displayNumber} · {x.statement.slice(0,80)}</option>)}</select></label>}
 ...
 {resolving.trigger!=='ProcedureOrphaned'&&<label>Procedure action<select name="procedureChangeAction" defaultValue={resolving.trigger==='RequirementIntroduced'?'CreateNew':'ModifyExisting'}><option value="CreateNew">Create a new test procedure</option><option value="LinkExisting">Link an existing procedure without change</option><option value="ModifyExisting">Modify an existing procedure</option><option value="RetireExisting">Retire an existing procedure</option></select></label>}
 {resolving.trigger!=='ProcedureOrphaned'&&<label className="preReleaseEvidenceChoice"><input type="checkbox" name="preReleaseEvidenceRequired"/><span><b>Evidence required before software-build release</b><small>Select only when this procedure must pass with captured evidence before release. Otherwise execution continues after release.</small></span></label>}
