...
export default function VerificationCenter({api,programId,projectId,releaseId,buildName,readOnly,scope,user,correctiveProblemReportId,onBack}:Props){
 ...
 const loadImpact=useCallback(async()=>{const response=await fetch(`${api}/api/releases/${releaseId}/verification-impact`);if(response.ok)setImpact(await response.json());else{recordClientOperationFailure('verification.impact.load',new Error(`HTTP ${response.status}`));setError('The verification impact for this build could not be loaded.')}},[api,releaseId])
 ...
 const loadReviews=useCallback(async()=>{const response=await fetch(`${api}/api/releases/${releaseId}/test-change-reviews`);if(response.ok)setTestChangeReviews(await response.json());else{recordClientOperationFailure('verification.reviews.load',new Error(`HTTP ${response.status}`));setError('The test change reviews for this build could not be loaded.')}},[api,releaseId])
