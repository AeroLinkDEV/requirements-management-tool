...
export default function VerificationCenter({api,programId,projectId,releaseId,buildName,readOnly,scope,user,correctiveProblemReportId,onBack}:Props){
 ...
 const loadReviews=useCallback(async()=>{const response=await fetch(`${api}/api/releases/${releaseId}/test-change-reviews`);if(response.ok)setTestChangeReviews(await response.json())},[api,releaseId])
