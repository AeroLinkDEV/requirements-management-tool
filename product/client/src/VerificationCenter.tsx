...
type TestChangeReview={id:string;discipline:string;state:string;sourceChangeRequestNumber:string;assignedEngineerId?:string;submittedBy?:string;submittedAt?:string;approvedBy?:string;approvedAt?:string;approvalRationale:string;totalItems:number;resolvedItems:number;preReleaseEvidenceItems:number}
  ...
  {workspaceTab==='impact'&&<section className="verificationCard">
   ...
   <section className="testChangeReviewList" aria-label="Test change reviews">
    {scopedReviews.map(review=><article className={`testChangeReview ${review.state.toLowerCase()}`} key={review.id}>
      <div><b>{review.sourceChangeRequestNumber}</b><i>{review.discipline==='HighLevelSoftware'?'Software HLR':review.discipline==='LowLevelSoftware'?'Software LLR':'System'} test change review</i><span>{review.state}</span></div>
