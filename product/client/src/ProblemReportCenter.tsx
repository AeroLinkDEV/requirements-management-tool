...
import { PROBLEM_REPORT_NARRATIVE } from "./problemReportFields";
import ProblemReportImpactPanel, { type ImpactArea } from "./ProblemReportImpactPanel";
import RelatedProblemReports, { type RelatedReport } from "./RelatedProblemReports";
import { useCategoryVocabulary, type SelectedCategory } from "./problemReportCategories";
  ...
  return <main className="problemReportsPage">
    ...
    <section className="prWorkspace"><aside><div className="prListHead"><div><h2>Problem Report queue</h2><small>{pageStart}–{pageEnd} of {reportTotal} matching records</small></div></div>
      ...
      <nav className="prTabs" aria-label="Problem Report sections"><button className={tab==="record"?"active":""} onClick={()=>setTab("record")}>Record</button><button className={tab==="history"?"active":""} onClick={()=>setTab("history")}>History <span>{selected.revisions?.length??0}</span></button></nav>
        ...
        <ProblemReportImpactPanel areas={selected.impactAreas??[]} narrative={selected.systemAircraftImpact??""} onOpen={onOpenArtifact}/>
        <RelatedProblemReports api={api} projectId={projectId} reportId={selected.id} related={selected.relatedReports??[]} canEdit={!isFinished} busy={busy} onLink={linkRelated} onUnlink={unlinkRelated} onOpen={id=>void open(id)}/>
