...
const problemReportDownload=(api:string,reportId:string,format:"docx"|"pdf",revision?:number)=>{
...
export default function ProblemReportCenter({api,projectId,releaseId,releases,user,initialReportId,onSelected,onBack,onOpenVerification,onOpenArtifact}:Props){
  ...
  return <main className="problemReportsPage">
    ...
    <section className="prWorkspace"><aside><div className="prListHead"><div><h2>Problem Report queue</h2><small>{pageStart}–{pageEnd} of {reportTotal} matching records</small></div></div>
      ...
      <nav className="prOutputActions" aria-label="Problem Report downloads"><span>Controlled output</span><a href={problemReportDownload(api,selected.id,"docx")} target="_blank" rel="noreferrer">Download DOCX</a><a href={problemReportDownload(api,selected.id,"pdf")} target="_blank" rel="noreferrer">Download PDF</a></nav>
