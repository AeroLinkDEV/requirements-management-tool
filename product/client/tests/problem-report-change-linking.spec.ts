...
test('a PR links to change requests and System TCRs without changing its lifecycle state', async ({ page, request }) => {
  ...
  const linkedItems = await linkedToChange.json()
  const linkedReport = linkedItems.find((item: { id: string }) => item.id === report.id)
  expect(linkedReport?.snapshotId).toBeTruthy()
  const drivingLink = page.locator('.artifactReferenceCard').filter({hasText:report.displayNumber})
  await expect(drivingLink).toHaveAttribute('href', new RegExp(`/problem-reports/${report.id}\\?snapshotId=${linkedReport.snapshotId}$`))
  await drivingLink.click()
  await expect(page).toHaveURL(new RegExp(`/problem-reports/${report.id}\\?snapshotId=${linkedReport.snapshotId}$`))
  ...
  expect(linkedItems.map((item: { id: string }) => item.id)).toContain(report.id)
  ...
  await expect(panel).toContainText('Verification For Problem')
  // The change request sits under System requirements, which is the area it changes.
  const correctiveAction = panel.locator('.impactRow').filter({ hasText: 'System requirements' })
    .locator('.impactArtifact').filter({ hasText: change.displayNumber })
  await expect(correctiveAction).toContainText(change.displayNumber)
  ...
  await expect(page.getByRole('region', { name: 'Impact and linked evidence' }))
    .toContainText('Verification For Problem')
  ...
  await expect(page.getByRole('region', { name: 'Impact and linked evidence' }))
    .not.toContainText('Proposed Corrective Action')
