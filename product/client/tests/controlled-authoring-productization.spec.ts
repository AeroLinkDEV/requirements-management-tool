...
test('an incomplete Draft can be saved and checked in while review remains gated',async ({page,request})=>{
  test.setTimeout(90_000)
  const programName=await createWorkspace(request,'Incomplete Draft')
  await selectProgram(page,programName)
  await openNewSoftwareChangeRequest(page,'HLR')
  ...
  await expect(page.getByRole('heading',{name:'Draft navigation behavior updated'})).toBeVisible()
  await expect(page.getByRole('button',{name:'Configure & Submit Review'})).toHaveCount(0)
  await expect(page.getByText('Draft needs authoring')).toBeVisible()
})
