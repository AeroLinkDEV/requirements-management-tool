...
test('Team Work is a project-wide three-lane board with API-owned card truth and canonical links', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Awaiting signature', exact: true })).toHaveCount(0)
  ...
  await expect(page.locator('[data-lane="review"]')).toContainText('Assessment of HLRCR-00002.00')
  ...
  await expect(parallelCard).toContainText('Review')
  await expect(parallelCard.locator('.teamWorkLanePill')).toHaveCount(0)
  await expect(parallelCard).not.toContainText('Native state')
  ...
  await expect(interfaceCard).not.toContainText('historical-author')
  await interfaceCard.locator('..').getByText('Details', { exact: true }).click()
  await expect(interfaceCard.locator('..')).toContainText('historical-author')
  ...
  await problemReportCard.locator('..').getByText('Details', { exact: true }).click()
  await expect(problemReportCard.locator('..')).toContainText('historical-reporter')
  ...
  await expect(problemReportCard.locator('..')).toContainText('Responsible engineer obligation')
  ...
  await expect(unheldCard).toContainText('Next action')
  ...
  await expect(deferredCard).toContainText('Deferred · not assigned')
  await deferredCard.locator('..').getByText('Details', { exact: true }).click()
  await expect(deferredCard.locator('..')).toContainText('Build 1.5')
...
test('Team Work preserves a numbered automatic TCR whose title was not persisted', async ({ page }) => {
  ...
  await card.locator('..').getByText('Details', { exact: true }).click()
  await expect(card.locator('..')).toContainText('Assigned engineer obligation')
  await expect(card.locator('..')).toContainText('source problem report')
})
...
test('Team Work keeps a board-shaped loading skeleton while the project projection is pending', async ({ page }) => {
  ...
  await expect(page.locator('.teamWorkBoardLoading .teamWorkLane')).toHaveCount(3)
  releaseResponse?.()
  await expect(page.getByRole('heading', { name: 'Team Work', exact: true })).toBeVisible()
})
...
test('Team Work derives contextual layers and artifact types without duplicating parallel items', async ({ page }) => {
  ...
  await expect(page.getByRole('combobox', { name: 'Artifact Type' })).toContainText('SRCR (1)')
...
test('Team Work replaces person selection, preserves shared-holder cards, and clears without hiding the board', async ({ page }) => {
  ...
  await expect(page.getByRole('combobox', { name: 'Artifact Type' })).toContainText('HLRCR (1)')
...
test('Team Work ranks current workloads before zero-work members and keeps build counts scoped', async ({ page }) => {
  ...
  await expect(page.locator('.teamWorkPerson').filter({ hasText: 'Busy Thirty Six' }).locator('.teamWorkLoadShape'))
    .toHaveAttribute('aria-label', '30 holds: 18 in work, 10 in review, 2 approved')
...
test('Team Work resolves drawer workload visuals in roster scope', async ({ page }) => {
  ...
  await expect(drawer.locator('.teamWorkDrawerLoadBar'))
    .toHaveAttribute('aria-label', '18 in work, 10 in review, 2 approved')
  ...
  await expect(drawer.locator('.teamWorkDrawerLoadBar'))
    .toHaveAttribute('aria-label', '2 in work, 3 in review, 1 approved')
})
...
test('Team Work names a known raiser without exposing their account handle', async ({ page }) => {
  ...
  await card.locator('..').getByText('Details', { exact: true }).click()
  await expect(card.locator('..').getByText('Systems Author', { exact: true })).toBeVisible()
  await expect(card.locator('..')).not.toContainText('systems.author')
