...
test('a configured two-stage sequential TCR review completes through the UI', async ({ page, browser, request }) => {
  ...
  const propose = page.getByRole('dialog', { name: 'Propose a procedure change' })
  ...
  await propose.getByLabel('Why this procedure work is required').fill('Nothing covers the new requirement.')
  ...
  const stageOneRow = stageOnePage.locator('.downstreamAssessment').filter({ hasText: /SYSTCR-/ })
  ...
  const stageTwoRow = stageTwoPage.locator('.downstreamAssessment').filter({ hasText: /SYSTCR-/ })
  ...
  const packageItem = reviews.items.find((item: { displayNumber: string }) => item.displayNumber.startsWith('SYSTCR-'))
