...
test('a team records its review procedure, puts it in force, and revises it without losing the prior version', async ({ page }) => {
  ...
  await composer.getByLabel('Stage 1 required project authority').selectOption('BaseRole:SystemEngineer')
  // The composer's second row arrives with an unselected authority on purpose: nothing is signed until an
  // explicit authority is chosen, and Save stays disabled until every row has one.
  await composer.getByLabel('Stage 2 required project authority').selectOption('BaseRole:ConfigurationManager')
  await composer.getByRole('button', { name: 'Add a stage' }).click()
  await composer.getByLabel('Stage 3 name').fill('Change board')
  await composer.getByLabel('Stage 3 signature').selectOption('Approval')
  await composer.getByLabel('Stage 3 required project authority').selectOption('LeadershipPosition:ConfigurationManager')
  ...
  await expect(systems.getByText('Review signature · System Engineer')).toBeVisible()
  await expect(systems.getByText('Approval signature · Configuration Manager — Project Leadership')).toBeVisible()
  ...
  await composer.getByLabel('Stage 3 required project authority').selectOption('BaseRole:ConfigurationManager')
  await composer.getByRole('button', { name: 'Save as draft' }).click()
