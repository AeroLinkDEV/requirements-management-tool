...
test('draft updates preserve controlled identities and normalize new proposals on the server',async({request})=>{
  ...
  expect(updated.requirementChanges[1].baseNumber).toMatch(/^SYSR-\d{6}$/);expect(updated.requirementChanges[1].baseNumber).not.toBe(original.baseNumber);expect(updated.requirementChanges[1].revision).toBe(0)
