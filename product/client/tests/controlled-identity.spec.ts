...
test('draft updates preserve controlled identities and normalize new proposals on the server',async({request})=>{
  ...
  const updatedResponse=await request.get(`${apiBase}/api/scrs/${scr.id}`);expect(updatedResponse.ok(),await updatedResponse.text()).toBeTruthy();const updated=await updatedResponse.json();expect(updated.requirementChanges).toHaveLength(3)
