...
test('Program scope protects dashboards, signatures, impacts, directories, and baseline views',async({request,playwright})=>{
 ...
 try{await approveShowcaseSystemFixture(author,signer,signedArtifact,'Approved as a tenant-isolation signature probe.')}
 finally{await signer.dispose();await author.dispose()}
