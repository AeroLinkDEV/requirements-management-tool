...
test('Program scope protects dashboards, signatures, impacts, directories, and baseline views',async({request,playwright})=>{
 ...
 const scoped=await playwright.request.newContext();const login=await scoped.post(`${apiBase}/api/auth/login`,{data:{userName,password}});expect(login.ok(),await login.text()).toBeTruthy()
