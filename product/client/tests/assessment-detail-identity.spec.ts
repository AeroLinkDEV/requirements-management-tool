...
for (const [discipline, artifactKind, outcome, state, label] of [
  ...
  await expect(page.locator('.sourceRecord').getByText('SRCR-00143.00', { exact: true })).toBeVisible()
  await expect(page.getByText('The exact source that raised this assessment', { exact: true })).toBeVisible()
  await expect(page.getByText('Draft needs authoring', { exact: true })).toHaveCount(0)
