...
test('FMS 1.5 released baseline supports active 1.6 work and full lifecycle exploration', async ({ page, request }) => {
  ...
  await page.getByRole('button', { name: 'Disposition impact' }).first().click()
  await page.getByPlaceholder('Required rationale and evidence of disposition').fill('Requirement impact reviewed and allocated to the controlled 1.6 work package.')
  await page.getByRole('button', { name: 'Mark addressed' }).click()
  await expect(page.getByText('23 pending')).toBeVisible()
})
