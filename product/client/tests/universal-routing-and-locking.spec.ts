...
test('deep links survive refresh, universal search resolves fragments, and checkout is read-only to another user',async({page,browser,request})=>{
 ...
 await page.keyboard.press('Control+K');await page.getByPlaceholder(/Search pages, SCRs/).fill('0150');await expect(page.getByRole('dialog').getByText(/SYSR-00000150\.\d{2}/)).toBeVisible()
