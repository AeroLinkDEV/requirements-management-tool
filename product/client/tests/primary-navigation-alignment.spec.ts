...
test('primary navigation separates generated data, managed documents, code and problem reporting without breaking historical routes',async({page,request})=>{
  ...
  const code=nav.locator('.navGroup').filter({has:page.locator('summary').filter({hasText:/^CODE$/})})
  ...
  await expect(code.getByRole('link',{name:'Code merge requests',exact:true})).toBeVisible()
  await expect(code.getByRole('link',{name:'Code explorer',exact:true})).toBeVisible()
  await expect(nav.locator('.navStandalone').getByRole('link',{name:'Code traceability'})).toHaveCount(0)
