...
test('a Modify target beyond the former 200-row limit is searchable, selectable and survives reload', async ({ page, request, playwright }) => {
  ...
  await assessment.getByRole('button', { name: 'LLRTCCR required', exact: true }).click()
  await expect(assessment).toContainText('LLRTCCR Created', { timeout: 30_000 })
  await assessment.getByRole('button', { name: /^LLRTCCR-\d{6}\.\d{2}$/ }).click()
  ...
  await reopenedRow.getByRole('button', { name: /^LLRTCCR-\d{6}\.\d{2}/ }).click()
...
test('a Modify target beyond the former 500 limit is reachable in the real picker and survives reload', async ({ page, request }) => {
  ...
  await assessment2.getByRole('button', { name: /^SYSTPCR-\d{6}\.\d{2}$/ }).click()
  ...
  await afterReload.getByRole('button', { name: /^SYSTPCR-\d{6}\.\d{2}/ }).click()
  const reopened = page.getByRole('dialog', { name: /procedure decisions/ })
