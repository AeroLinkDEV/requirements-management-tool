...
public sealed class ConfigurationPublicationApiTests(ShowcaseApiFixture showcase)
{
    ...
    {
        using var factory=showcase.CreateFactory(enableEnterpriseJobWorker:true);using var client=factory.CreateClient();await BootstrapAsync(client);Guid projectId,activeBaselineId,releasedBaselineId,templateRevisionId;
    ...
    // The timeout reports the job rather than only the fact of the timeout. "did not complete" says nothing
    // about whether it was never claimed, is still running, or failed in a way the poll missed.
    private static async Task<JsonElement> Completed(HttpClient client,Guid id){JsonElement last=default;for(var i=0;i<80;i++){var job=await client.GetFromJsonAsync<JsonElement>($"/api/publications/jobs/{id}");last=job;var state=job.GetProperty("state").GetString();if(state=="Completed")return job;if(state=="Failed")Assert.Fail(job.GetProperty("lastError").GetString());await Task.Delay(250);}throw new TimeoutException($"Job {id} did not complete. Last seen: {last}");}
