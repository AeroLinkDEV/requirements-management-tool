...
public sealed class DeferralAndRevisionListingTests
{
    private static async Task<(Guid ProjectId, Guid ReleaseId, Guid ScrId)> SeedAsync(AeroLinkApiFactory factory)
    {
        ...
        var scr = new SystemChangeRequest("SCR-00000070", 0, project.Id, release.Id,
    ...
    public async Task An_approved_change_request_goes_on_the_shelf_and_comes_back_where_it_was()
    {
        ...
        using var deferred = await client.PostAsJsonAsync($"/api/scrs/{scrId}/defer",
        ...
        using var reinstated = await client.PostAsync($"/api/scrs/{scrId}/reinstate", null);
    ...
    public async Task Deferring_without_a_reason_is_refused()
    {
        ...
        using var response = await client.PostAsJsonAsync($"/api/scrs/{scrId}/defer", new { reason = "   " });
    ...
    public async Task The_listing_collapses_to_the_newest_revision_and_expands_on_request()
    {
        ...
        using var revised = await client.PostAsJsonAsync($"/api/scrs/{scrId}/next-revision", new { });
        Assert.Equal(HttpStatusCode.Created, revised.StatusCode);

        using var collapsed = await client.GetAsync($"/api/history/scrs?projectId={projectId}&page=1&pageSize=50");
        ...
        Assert.Equal("SCR-00000070.01", row.GetProperty("displayNumber").GetString());
        ...
        using var expanded = await client.GetAsync(
            $"/api/history/scrs?projectId={projectId}&baseNumber=SCR-00000070&page=1&pageSize=50");
        ...
        Assert.Equal(["SCR-00000070.00", "SCR-00000070.01"], numbers);
    }
}
