...
public sealed class ShowcaseSeedApiTests
{
    ...
    public async Task Showcase_seed_endpoint_builds_the_controlled_dataset_on_disposable_sqlite()
    {
        ...
        Assert.True(overview.GetProperty("testArtifacts").GetInt32() > 0);
        Assert.Equal(overview.GetProperty("testProcedures").GetInt32(), overview.GetProperty("testArtifacts").GetInt32());
