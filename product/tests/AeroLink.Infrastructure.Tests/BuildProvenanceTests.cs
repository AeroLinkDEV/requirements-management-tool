    ...
    public async Task Build_retains_exact_frozen_baseline_and_selected_scr_history()
    {
        ...
        try
        {
            ...
            await using (var db = new AeroLinkDbContext(options))
            {
                ...
                var scr = new SystemChangeRequest("SWCR-00042", 1, project.Id, release.Id, "Round robin", "P", "A", "S", "author", now, ChangeRequestType.Software);
            ...
            await using (var verify = new AeroLinkDbContext(options))
            {
                ...
                Assert.Equal("SWCR-00042.01", scr.DisplayNumber); Assert.Equal("SWR-00002375.04", scr.RequirementChanges.Single().DisplayNumber);
            }
        }
