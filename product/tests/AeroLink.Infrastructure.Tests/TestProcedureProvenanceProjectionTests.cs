...
public sealed class TestProcedureProvenanceProjectionTests
{
    ...
    public async Task Folded_sources_keep_each_exact_change_request_and_the_exact_package_revision()
    {
        ...
        var now = fixture.Now;
        var crA = ChangeRequest("SRCR-004240", fixture.Project.Id, fixture.Release.Id, "Primary", now);
        var crB = ChangeRequest("SRCR-004241", fixture.Project.Id, fixture.Release.Id, "Folded", now);
        var changeA = AddRequirement(crA, "SYSR-004240", now);
        var changeB = AddRequirement(crB, "SYSR-004241", now);
        var tcr = new TestChangeReview(fixture.Project.Id, fixture.Release.Id, crA.Id,
            ...
            baseNumber: "SYSTCR-004240", revision: 1);
        tcr.IncludeChangeRequest("verification.engineer", crB.Id, crB.DisplayNumber, now);
        var procedure = new TestProcedure(fixture.Project.Id, "SYSTP-004240", "Folded source procedure",
        ...
        Assert.Equal("SYSTCR-004240.01", provenance.Package);
        Assert.False(provenance.IsLegacy);
        Assert.Null(provenance.Note);
        Assert.Equal(2, provenance.Drivers.Count);
        Assert.All(provenance.Drivers, row => Assert.Equal("SYSTCR-004240.01", row.Package));
        Assert.Equal(new[] { "SRCR-004240.00", "SRCR-004241.00" },
    ...
    public async Task Manual_tcr_without_impacts_keeps_the_package_and_all_claimed_source_changes()
    {
        await using var fixture = await Fixture.CreateAsync();
        var now = fixture.Now;
        var primary = ChangeRequest("SRCR-004250", fixture.Project.Id, fixture.Release.Id, "Manual primary", now);
        var folded = ChangeRequest("SRCR-004251", fixture.Project.Id, fixture.Release.Id, "Manual folded", now);
        var tcr = new TestChangeReview(fixture.Project.Id, fixture.Release.Id, primary.Id,
            ...
            baseNumber: "SYSTCR-004250", revision: 0);
        tcr.IncludeChangeRequest("verification.engineer", folded.Id, folded.DisplayNumber, now);
        var procedure = new TestProcedure(fixture.Project.Id, "SYSTP-004250", "Manual package procedure",
            "verification.engineer", now, TestProcedureLevel.System);
        ...
        Assert.Equal("SYSTCR-004250.00", provenance.Package);
        Assert.Equal(2, provenance.Drivers.Count);
        Assert.All(provenance.Drivers, row =>
        {
            Assert.Equal("SYSTCR-004250.00", row.Package);
            Assert.Equal("PackageSource", row.Action);
            Assert.Equal("", row.SubjectDisplayNumber);
            Assert.False(row.IsLegacy);
        });
        Assert.Equal(new[] { "SRCR-004250.00", "SRCR-004251.00" },
            provenance.Drivers.Select(x => x.ChangeRequest).OrderBy(x => x).ToArray());
    }
    ...
    {
        await using var fixture = await Fixture.CreateAsync();
        var procedure = new TestProcedure(fixture.Project.Id, "SYSTP-004260", "Legacy procedure",
