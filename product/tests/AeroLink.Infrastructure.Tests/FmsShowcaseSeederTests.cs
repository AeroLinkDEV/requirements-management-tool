...
public sealed class FmsShowcaseSeederTests
{
    ...
    public async Task Generates_exact_released_15_baseline_and_active_16_work_without_duplicates()
    {
        ...
        try
        {
            ...
            Assert.Equal(8 + FmsShowcaseSeeder.ActiveTraceRequestCount, await active.CountAsync()); Assert.Equal(2, await active.CountAsync(x => x.State == ChangeRequestState.SelectedForBaseline));
    ...
    public async Task Fresh_identity_ordering_freezes_real_sqa_identity_and_covers_scenario_lifecycles()
    {
        ...
        try
        {
            ...
            Assert.Equal(8 + FmsShowcaseSeeder.ActiveTraceRequestCount, await db.SystemChangeRequests.CountAsync(x => x.ProjectId == summary.ProjectId && x.TargetReleaseId == summary.ActiveReleaseId));
