...
public sealed class ReqIfExchangeTests
{
    ...
    public async Task Exported_package_round_trips_identifiers_hierarchy_relations_and_content()
    {
        ...
        try
        {
            ...
            var service=new ReqIfExchangeService(db,new EvidenceFileStore(evidence));var exported=await service.ExportAsync(project.Id,"author",now,CancellationToken.None);
