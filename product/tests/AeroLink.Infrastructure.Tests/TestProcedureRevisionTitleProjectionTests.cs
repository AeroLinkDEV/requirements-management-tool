...
public sealed class TestProcedureRevisionTitleProjectionTests
{
    ...
    public async Task Exact_tcr_titles_survive_a_later_catalog_mutation_and_retirement_keeps_its_predecessor_title()
    {
        ...
        var change = new SystemChangeRequest("SRCR-004210", 0, project.Id, release.Id,
            "Title change", "Problem", "Analysis", "Solution", "author", now);

        var procedure = new TestProcedure(project.Id, "SYSTP-004210", "Verify legacy route sequencing",
        ...
        var retire = Review(project.Id, release.Id, change.Id, change.DisplayNumber,
            "SYSTCR-004210", 2, TestProcedureChangeKind.Retire, "", now.AddMinutes(2));
    ...
    [Fact]
    public async Task Legacy_revision_uses_catalog_title_with_truthful_compatibility_wording()
    {
        await using var connection = new SqliteConnection("Data Source=:memory:");
        await connection.OpenAsync();
        var options = new DbContextOptionsBuilder<AeroLinkDbContext>().UseSqlite(connection).Options;
        ...
        var procedure = new TestProcedure(project.Id, "SYSTP-004211", "Legacy catalog title",
        ...
        Assert.Equal("Legacy catalog title", title.Title);
        ...
        string title, DateTimeOffset now)
    {
        ...
        review.AddProcedureChange("verification.engineer", new TestProcedureChangeDraft("SYSTP-004210",
