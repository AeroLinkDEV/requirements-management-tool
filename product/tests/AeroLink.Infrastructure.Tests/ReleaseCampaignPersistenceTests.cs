...
public sealed class ReleaseCampaignPersistenceTests(ShowcaseDatabaseFixture showcaseFixture)
{
    ...
    public async Task Showcase_campaign_has_real_gates_impacts_outputs_and_checksummed_evidence()
    {
            ...
            // The in-work build's campaign specifically: the released build now has its own closed one.
        {
            ...
            Assert.Equal(32 + FmsShowcaseSeeder.ActiveTraceRequestCount * 4, await db.ImpactDispositions.CountAsync(x => x.CampaignId == campaign.Id)); Assert.Equal(8, await db.ImpactDispositions.CountAsync(x => x.CampaignId == campaign.Id && x.State == ImpactDispositionState.Addressed));
            var readiness = await new ReleaseReadinessService(db).CalculateAsync(campaign.Id, default); Assert.False(readiness.ReadyForRelease); Assert.Contains(readiness.Gates, x => x.Code == "change_control" && x.Completed == 2 && x.Total == 7 + FmsShowcaseSeeder.ActiveTraceRequestCount);
