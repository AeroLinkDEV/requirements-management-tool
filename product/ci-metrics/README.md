...
counts as product. Any doubt keeps the full gate set. A derived documentation-only candidate binds on either
topology: the complete successful gate set (a run that classified broad), or the documentation topology, in
which every required job appears exactly once and concludes `skipped`, with no expanded shard and exactly one
