...
test('planner hash is stable and covers its own metadata contract', () => {
  ...
  assert.ok(PLANNER_FILES.includes('.github/workflows/ci.yml'), 'the workflow that supplies CI conditions must be hashed')
  ...
  try {
    ...
    const beforeWorkflowChange = plannerHash(temporaryRoot)
    appendFileSync(join(temporaryRoot, '.github/workflows/ci.yml'), '\n# deterministic workflow fixture change\n')
    assert.notEqual(plannerHash(temporaryRoot), beforeWorkflowChange, 'workflow condition changes must change the planner hash')
  } finally {
