...
test('scoped backend jobs build their graphs while retained owners build the full solution', () => {
  ...
  const domainJobConfig = domain.slice(0, domain.indexOf('    steps:'))
  assert.doesNotMatch(domainJobConfig, /continue-on-error:/, 'the required Domain job cannot be failure-isolated')
  const domainBuildStart = domain.indexOf('      - name: Restore and build the solution')
  const domainBuildEnd = domain.indexOf('      - name: Mark setup complete', domainBuildStart)
  assert.ok(domainBuildStart >= 0 && domainBuildEnd > domainBuildStart, 'Domain restore/build step must remain identifiable')
  const domainBuild = domain.slice(domainBuildStart, domainBuildEnd)
  assert.doesNotMatch(domainBuild, /continue-on-error:/, 'the required whole-solution build cannot be failure-isolated')
  assert.doesNotMatch(domainBuild, /--no-dependencies/, 'the whole-solution owner must retain dependency traversal')
