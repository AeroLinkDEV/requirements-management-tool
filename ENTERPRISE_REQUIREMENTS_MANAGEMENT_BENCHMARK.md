...
**Implementation checkpoint — 2026-07-12:** Wave 1 now has a production-shaped controlled-authoring and impact-analysis release. In addition to persisted schemas, specifications, discovery, history, redlines, comments, saved views, bulk jobs, and governed CSV/XLSX preview, engineers can start from an approved requirement, inspect its relationship/verification/baseline/build/document/change/collaboration impact, and create the proposed next revision inside a Draft SCR/SWCR. Structured rich content, Program fields, explicit impact dispositions, watchers, assignments, notifications, work queues, advanced structured filters, reusable import mappings, error CSVs, and persistent job history are integrated. A dedicated PostgreSQL qualification run materialized 10,000 mixed-level requirements in 7.5 seconds; warm p95 was 21 ms for the exact-current 100-row workspace query, 6 ms for a structured level/verification filter, and 2 ms for specification-tree aggregation. Remaining Wave 1 depth includes controlled images/attachments, true concurrent-editor merge assistance, shareable URL/query-builder polish, background import retry/export, and browser/concurrency performance qualification.
