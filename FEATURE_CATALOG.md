...
The connected-foundation increment establishes the first production slice of API-001, API-002, and INT-003: project-scoped machine identities, `/api/v1` requirements reads, idempotent event ingestion, per-credential rate limiting, durable events and deliveries, HMAC-signed webhooks, retry/backoff, dead-letter replay, and an operator-facing Integration Command Center. These catalog items remain open until the full acceptance criteria above are met, including conditional requirement writes, complete lifecycle event coverage, compatibility policy, monitored connector checkpoints, and ReqIF 1.2 round-trip.
