...
### DEC-127 - Digital Thread Hover Is Stationary Emphasis and Only Out-of-View Linked Cards Are Displaced

- **Date:** 2026-09-11
- **Status:** Accepted by the product owner, 2026-09-11, issue #1022.
- **Supersedes:** DEC-126's Digital Thread hover clauses and #1016 S10's permission to preview a different card while a subject is selected, together with every interpretation that permitted hover to change zoom or pan, rearrange whole lanes or already-visible cards, require a pin-story popup, or restore an old camera after the reader navigated. DEC-126's unrelated Team Work decisions and the accepted trace, identity and provenance rules are unchanged. On this surface it also reconciles the absolute framing expectations previously taken from #880 §6.4 and §6.6.
- **Decision:** Hover emphasises the exact subject and its directed thread and causes **no automatic camera movement** — no pan, zoom, fit, recenter, density change or indirect shift of reserved space. Only records with **no measured intersection** with the usable window receive a temporary displacement, and only vertically within their own lane: a placed card is fully inside the usable window or directly below it, never above it (that would need a scroll the reader's lane cannot supply), and never over a card that stays put. The source card under the pointer and every visible or partially visible record keep their positions. Before a selection, hover may prepare lanes the camera is not showing, including lanes beyond the horizontal edge; the camera is never moved to them and no card changes lane.
- **Selection:** A single click selects the exact record — hovered or direct — and retains the displayed arrangement without first rebuilding it; a record that was pulled into view as a linked card keeps its displayed position when it becomes the subject, rebased against the current measured layout and lane scroll. A persistent selection owns the thread, the tracing and the tray; pointer movement across any other card can never preview or replace it, and pointer leave cannot clear it. Clearing is the reader's explicit action and holds the current camera. The selected root may leave the viewport by the reader's own navigation without losing selection.
- **Navigation and geometry:** The lane's permitted scroll range is derived from the complete **displayed** arrangement, including temporary displacements, and a bounded in-memory residual allowance keeps a reader who scrolled into that temporary range from being snapped back when temporary geometry is removed; the allowance is released by the reader's own navigation and dropped on a scope change. It is never persisted layout. Automatic whole-lane alignment on selection is superseded by per-card reveal. Deliberate lane scrolling, explicit `Show <ID>`/keyboard reveal and explicit Fit commands remain user navigation. A record that is off-screen is indicated by directional continuation at the real usable boundary plus its truthfully labelled reveal action — never by moving unrelated records, and never by presenting it as fitted when it is not.
- **Framing and motion:** Automatic click-time framing is bounded: it corrects the camera only when the selected record is not fully usable, and the readability floors of DEC-117 (landing) and DEC-120 (selection) still govern. Explicit Fit commands always do what they say. Manual input takes the camera from its currently painted position, and passive measurement, tray insets and re-renders never re-land a board the reader owns. Reduced motion keeps the same final geometry without animation.
- **Consequences:** The previous floating preview target, its "Click to pin story" wording and its saved-camera restore are removed rather than restyled. Counts are not the primary overflow cue on this surface. Filtered, unauthorized, missing or truncated records remain a different state from viewport overflow and keep their truthful notices. This decision changes presentation and navigation only: exact revision identity, typed relationship direction, bounded traversal and controlled history are untouched.

### DEC-128 - Project Inception Separates Source Assertions from the New Working Build

- **Date:** 2026-09-14 UTC
- **Status:** Accepted owner policy from [issue #1037](https://github.com/AeroLinkDEV/requirements-management-tool/issues/1037), including its eleven owner answers and the 2026-09-13 implementation authorization. The source-eligibility and persistence details below implement that policy and were independently challenged before implementation.
- **Supersedes:** DEC-093's requirement that an external import create a released build applies to its existing-project legacy import path. New-project inception instead creates the owner's chosen first **IN WORK** build. DEC-093's exact provenance, independent source history, source assertion and no-fictitious-approval rules remain authoritative. Native AeroLink inheritance is an explicit native source origin, never described as an external extract or a newly approved change request.
- **Source and target:** A native source baseline remains in its source project. An authorized Frozen or Released baseline with the applicable materialized manifest can supply exact selected revisions; a source need not be Released. The selected manifest, revisions and actual source lifecycle are captured and revalidated. A Frozen source never acquires released/settled status through inheritance. Source selection is separate from same-project build ancestry, and no duplicate source build is universally required. Destination artifacts receive their own controlled identities, retaining exact source artifact/revision identities and source facts permanently.
- **Recoverable staging:** Setup owns uploads, observations, mapping and reconciliation before the destination project exists. Project-bound import ledgers and materialized content are created within the short completion transaction after target identity is established. The accepted ladder is effective before inherited content seals it. Server-side reconciliation accounts for every source object, attribute and relationship; the browser cannot attest that its own mapping passed.
- **Acceptance meaning:** The actual authorized creator/administrator completing setup confirms their password and signs the exact source assertion: selected source identity/hash and revisions, included/excluded categories, mapping and reconciliation, accepted ladder and reserved destination identities. The assertion says what was inherited and how it was interpreted; it is not an engineering approval or test execution. An administrator who resumes another creator's draft signs as themselves. Native source access is required separately from setup management authority and is revalidated before completion.
...
### DEC-129 - Digital Thread Foreground Reveal Takes Priority over Background Cards

- **Date:** 2026-09-14
- **Status:** Accepted owner presentation policy in [issue #1046](https://github.com/AeroLinkDEV/requirements-management-tool/issues/1046). Implementation and runtime acceptance remain separately evidenced by the issue and its protected delivery.
- **Supersedes narrowly:** DEC-127's prohibition on temporary reveal over stationary background cards, its no-intersection eligibility and blanket partially-visible-card immobility, and its Show-ID actions and additional selected-state strip reservation. The hovered source remains stationary; this permission concerns linked foreground cards and selection-induced correction, not moving the source under a stationary pointer.
- **Foreground priority:** The authoritative directed story uses temporary vertical offsets within existing lanes. Measured emphasized cards may cover non-emphasized cards, whose canonical rows remain unchanged. Foreground cards protect one another's usable content. Already useful positions are retained where possible; a clipped linked card may move fully into the usable window when it fits. Hover never changes camera, zoom, density or whole-lane arrangement.
- **Selection and measurement:** A click selects the exact painted record and retains the useful arrangement, then reconciles selected and linked foreground cards against the actual inspector, measured expanded heights and usable window. The selected subject has correction priority. Internal route feedback must preserve this selection transaction; external arrival and browser history retain readable arrival framing. New measurements may repair a hover arrangement while keeping its source anchored. They must not recenter or clamp reader-owned navigation.
- **Partial fit and navigation:** Preserve necessary anchors, then try remaining candidates deterministically in lane row/identity order using measured legal slots. This is stable presentation order, not graph distance or relevance. A non-fitting or oversized candidate cannot strand smaller fitting candidates. New overflow stays legally reachable below the window; actual up/down continuation reflects the painted arrangement during motion and exploration. Remove the Show-ID strip without an equivalent space reservation or mandatory recovery step. Keyboard focus/arrow navigation, native controls, touch, lane scrolling and explicit Fit remain deliberate navigation.
- **Interaction and ownership:** Foreground stacking wins actual overlap. Focusability follows painted/clipped card and control rectangles; unobscured backgrounds remain usable. Stationary pointer coordinates do not retarget hover because another card moved beneath them. Deliberate movement and clicks remain active during animation. Persistent selection owns the story until explicit clear or another selection. Manual navigation may intentionally leave the subject offscreen; clear retains the current camera, including any required in-memory scroll allowance.
- **Unchanged boundaries:** Keep the shared canvas and geometry across Network, Inside and Artifact views, existing readability floors, real toolbar/heading/inspector clearance, and reduced-motion final geometry. Exact identities, server-owned relationships, authorization, controlled history and persisted layout are unchanged. Viewport overflow remains distinct from unavailable, filtered, unauthorized or truncated data.
...
- **Date:** 2026-09-25
- **Status:** Superseded by DEC-142 before it was installed.
...
- **Superseded by:** DEC-142. The owner did not want agents to need each other's agreement before a merge.
...
- **Kept:**
  - After each approval-machinery merge, the owner is notified with the exact change, and a check confirms that the `main` ruleset has no bypass actors.
